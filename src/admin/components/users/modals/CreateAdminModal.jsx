      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-white dark:bg-surface-dark rounded-[32px] shadow-2xl overflow-hidden flex flex-col animate-in fade-in zoom-in-95 duration-300">
        <div className="p-8 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
             <div className="h-10 w-10 bg-[#4C1D95]/5 dark:bg-[#4C1D95]/20 text-[#4C1D95] rounded-xl flex items-center justify-center">
                <Shield size={24} />
             </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-10 custom-scrollbar">
          ...
          <div className="space-y-6">
            <h3 className="text-xs font-black uppercase tracking-widest text-[#4C1D95] flex items-center gap-2">
               <div className="h-1 w-4 bg-[#4C1D95] rounded-full" />
            ...
            <div className="flex flex-col md:flex-row gap-10 items-start">
               ...
               <div className="relative group shrink-0 self-center md:self-start">
                  ...
                  <label className="absolute -bottom-2 -right-2 h-10 w-10 bg-[#4C1D95] text-white rounded-2xl flex items-center justify-center shadow-lg cursor-pointer hover:bg-[#22C55E] transition-all">
                     <UploadCloud size={18} />
               ...
               <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                  <div className="space-y-1.5">
                    ...
                    <input required type="text" className="w-full h-12 px-4 bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-xl text-sm font-bold outline-none focus:border-[#4C1D95]/20 transition-all" value={formData.firstName} onChange={e => setFormData({...formData, firstName: e.target.value})} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Last Name</label>
                    <input required type="text" className="w-full h-12 px-4 bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-xl text-sm font-bold outline-none focus:border-[#4C1D95]/20 transition-all" value={formData.lastName} onChange={e => setFormData({...formData, lastName: e.target.value})} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Corporate Email</label>
                    <input required type="email" className="w-full h-12 px-4 bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-xl text-sm font-bold outline-none focus:border-[#4C1D95]/20 transition-all" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                  </div>
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black uppercase tracking-widest text-zinc-400 ml-1">Phone Number</label>
                    <input required type="tel" className="w-full h-12 px-4 bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-xl text-sm font-bold outline-none focus:border-[#4C1D95]/20 transition-all" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                  </div>
               </div>
            </div>
          </div>
               ...
               {Object.entries(ADMIN_ROLES).map(([id, role]) => (
                 <button
                    ...
                    className={cn(
                      ...
                      formData.role === id 
                        ? "border-[#4C1D95]/20 bg-[#4C1D95]/5/50 dark:bg-[#4C1D95]/10" 
                        : "border-zinc-100 dark:border-zinc-800 hover:border-[#4C1D95]/20"
                    )}
                 >
                    <div className={cn("h-10 w-10 rounded-xl flex items-center justify-center mb-4 transition-transform group-hover:scale-110", formData.role === id ? "bg-[#4C1D95] text-white" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-400")}>
                       <role.icon size={20} />
                    </div>
                            ...
                            <label key={p.id} className="flex items-center gap-3 cursor-pointer group">
                               <div 
                                  className={cn(
                                    ...
                                    formData.permissions.includes(p.id) ? "bg-[#4C1D95] border-[#4C1D95]/20 text-white" : "border-zinc-200 dark:border-zinc-700"
                                  )}

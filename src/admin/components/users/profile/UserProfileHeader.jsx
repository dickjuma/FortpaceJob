...
const UserProfileHeader = ({ user }) => {
  ...
  return (
    <div className="bg-white dark:bg-surface-dark border border-zinc-100 dark:border-zinc-800 rounded-[40px] p-8 lg:p-10 shadow-sm relative overflow-hidden group">
      ...
      <div className="absolute top-0 right-0 h-64 w-64 bg-[#4C1D95]/5 rounded-full -mr-32 -mt-32 blur-3xl" />
      <div className="absolute bottom-0 left-0 h-48 w-48 bg-[#4C1D95]/5 rounded-full -ml-24 -mb-24 blur-2xl" />
      ...
      <div className="flex flex-col xl:flex-row items-center xl:items-start gap-10 relative z-10">
        ...
        <div className="flex flex-col items-center xl:items-start text-center xl:text-left gap-6 shrink-0">
          <UserAvatar 
            ...
            size="2xl" 
            className="shadow-2xl shadow-[#4C1D95]/25/10"
          />
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-y-6">
              {user.userGroup === 'freelancer' && (
                <>
                  ...
                  <HeaderStat icon={Briefcase} label="Contracts" value={user.completedContracts} color="text-[#4C1D95]" />
                  <HeaderStat icon={UserCheck} label="Success" value={`${user.successRate}%`} color="text-[#4C1D95]" />
                </>
              )}
              {user.userGroup === 'client' && (
                <>
                  <HeaderStat icon={DollarSign} label="Total Spend" value={formatCurrency(user.totalSpend)} color="text-[#4C1D95]" />
                  <HeaderStat icon={Briefcase} label="Jobs Posted" value={user.totalJobsPosted} color="text-[#4C1D95]" />
                  <HeaderStat icon={UserCheck} label="Hire Rate" value={`${user.hireRate}%`} color="text-success" />
                </>
              )}
              {user.userGroup === 'admin' && (
                <>
                  <HeaderStat icon={ShieldCheck} label="Actions Today" value={user.actionsToday} color="text-[#4C1D95]" />
                  <HeaderStat icon={Activity} label="Login Count" value={user.loginCount} color="text-[#4C1D95]" />
                </>
              )}
            </div>
            ...
            <div className="flex items-center gap-3 flex-wrap justify-center xl:justify-end">
              {publicUrl && (
                <Link
                  ...
                  className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl border border-zinc-200 dark:border-zinc-700 text-xs font-black uppercase tracking-widest text-[#4C1D95] hover:bg-[#4C1D95]/5 transition-colors"
                >

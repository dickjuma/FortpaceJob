  ...
  <div className="flex flex-col gap-1.5 min-w-[140px]">
    ...
    <div className="relative group">
      <select
        ...
        className="w-full h-10 pl-3 pr-8 bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-xl text-xs font-bold appearance-none outline-none focus:border-brand-500 transition-all cursor-pointer"
      >
    ...
    <div className="flex flex-col gap-6 mb-8 animate-in fade-in duration-500">
      <div className="flex flex-col lg:flex-row items-end gap-4">
        ...
        <div className="flex flex-col gap-1.5 flex-1 w-full lg:w-auto">
          ...
          <div className="relative group">
            {isDebouncing ? (
              <Loader2 className="absolute left-4 top-1/2 -tranzinc-y-1/2 text-brand-600 animate-spin" size={18} />
            ) : (
              <Search className="absolute left-4 top-1/2 -tranzinc-y-1/2 text-zinc-400 group-focus-within:text-brand-600 transition-colors" size={18} />
            )}
            <input
              type="text"
              ...
              className="w-full h-12 pl-12 pr-10 bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-2xl text-sm font-medium outline-none focus:border-brand-500 dark:focus:border-brand-500 focus:bg-white dark:focus:bg-surface-dark transition-all shadow-sm"
            />
            {localSearch && (
              <button 
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center bg-surface dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-800 rounded-xl overflow-hidden h-10 shadow-sm">
              ...
              <button
                ...
                className="px-3 border-l border-zinc-200 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors text-zinc-500 hover:text-brand-600"
              >
      ...
      <div className="flex flex-wrap items-center gap-4 border-t border-zinc-50 dark:border-zinc-800/50 pt-6">
        <div className="flex items-center gap-2 px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800/50 rounded-lg text-zinc-500">
           ...
           {activeCount > 0 && (
             <span className="h-4 min-w-[16px] px-1 flex items-center justify-center bg-brand-600 text-white text-[9px] font-black rounded-full">
               {activeCount}
             </span>
           )}
        </div>

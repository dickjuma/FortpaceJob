      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="h-[300px] flex items-end justify-between gap-2 px-4">
            {[45, 52, 48, 70, 65, 85].map((h, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-4 group">
                <div 
                  className="w-full bg-[#4C1D95]/20 group-hover:bg-[#4C1D95]/40 rounded-t-xl transition-all relative overflow-hidden" 
                  style={{ height: `${h}%` }}
                >
                  <div className="absolute top-0 left-0 w-full h-1 bg-[#4C1D95]" />
                </div>
        ...
        <div className="bg-surface-dark rounded-[32px] p-8 text-white relative overflow-hidden">
          <div className="relative z-10">
            <PieChart className="text-[#4C1D95] mb-6" size={32} />
            <h3 className="text-xl font-bold mb-4">Revenue Breakdown</h3>
            <div className="space-y-4">
              {[
                { label: 'Project Escrow Fees', percent: 45, color: 'bg-[#4C1D95]' },
                { label: 'Subscriptions', percent: 35, color: 'bg-success' },
                { label: 'Withdrawal Charges', percent: 20, color: 'bg-[#4C1D95]' },
              ].map((item, i) => (
          ...
          {/* Decorative glow */}
          <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-[#4C1D95]/20 rounded-full blur-3xl" />
        </div>
      </div>
    </div>
  );

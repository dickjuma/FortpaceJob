    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-surface-dark rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm flex flex-col">
        ...
        {selectedIds.length > 0 && (
          <div className="bg-[#2bb75c]/5 dark:bg-[#2bb75c]/10 border-b border-[#2bb75c]/20 dark:border-[#2bb75c]/20/20 p-3 flex items-center justify-between px-6">
            <span className="text-sm font-medium text-[#2bb75c] dark:text-[#2bb75c]">
            ...
            <thead>
              <tr className="bg-surface dark:bg-zinc-800/50 text-zinc-500 dark:text-zinc-400 text-xs uppercase tracking-wider font-semibold">
                <th className="p-4 w-12 text-center">
                  <input type="checkbox" onChange={toggleAll} checked={selectedIds.length > 0 && selectedIds.length === reviewsData?.data?.length} className="rounded border-zinc-300 dark:border-zinc-600 text-[#2bb75c] focus:ring-[#2bb75c]" />
                </th>
                ...
                reviewsData?.data?.map(review => (
                  <tr key={review.id} className={cn("hover:bg-surface/50 dark:hover:bg-zinc-800/20", review.flagged && "bg-red-50/30 dark:bg-red-900/10")}>
                    <td className="p-4 text-center align-top">
                      <input type="checkbox" checked={selectedIds.includes(review.id)} onChange={() => toggleSelect(review.id)} className="rounded border-zinc-300 dark:border-zinc-600 text-[#2bb75c] mt-2" />
                    </td>
                    ...
                    <td className="p-4 align-top">
                      ...
                      >
                        ...
                        <div>
                          <p className="text-sm font-semibold text-zinc-900 dark:text-white group-hover:text-[#2bb75c] transition-colors">{review.reviewerName}</p>

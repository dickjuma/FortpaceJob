...
import { useFreelancers } from '../../hooks/users/useFreelancers';
...
/**
 * Specialized directory for Freelancer accounts.
 */
const FreelancersPage = () => {
  const { data, isLoading, error } = useFreelancers();
  ...
  return (
    <div className="space-y-8 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black text-zinc-900 dark:text-white tracking-tight">Freelancers Directory</h1>
          <p className="text-zinc-500 font-medium mt-1">Manage talent pool, verify identities, and monitor freelance platform earnings.</p>
        </div>
        <Button variant="secondary" leftIcon={<Download size={18} />}>Export Talent List</Button>
      </div>
      ...
      <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-100 dark:border-zinc-800 shadow-sm overflow-hidden">
         <div className="p-6 border-b border-zinc-100 dark:border-zinc-800">
            <UserTableFilters section="freelancers" />
            <BulkActionsBar />
         </div>
         <div className="overflow-x-auto custom-scrollbar">
           <UserTable users={data?.data} loading={isLoading} error={error} section="freelancers" />
         </div>
         <TablePagination section="freelancers" totalCount={data?.total || 0} />
      </div>
    </div>
  );
};

  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans animate-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          { label: 'Active Projects', value: projects.length, icon: Briefcase, color: 'text-accent-purple', bg: 'bg-accent-purple/10' },
          ...
          { label: 'Average Progress', value: `${Math.round(projects.reduce((acc, p) => acc + p.progress, 0) / projects.length)}%`, icon: Clock, color: 'text-brand-500', bg: 'bg-brand-50' },
          { label: 'Risk Flags', value: projects.filter(p => p.health !== 'On Track').length, icon: AlertTriangle, color: 'text-accent-red', bg: 'bg-accent-red/10' }
        ].map((stat, idx) => (
          ...
          <Card key={project.id} className="bg-white border border-border rounded-3xl p-6 shadow-md hover:shadow-lg transition-all relative flex flex-col justify-between min-h-[320px]">
            <div>
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={cn(
                  ...
                  project.health === 'On Track' ? 'bg-success/15 text-success border-success/20' : 'bg-accent-red/10 text-accent-red border-accent-red/20'
                )}>
                  {project.health}
                </span>
              </div>
              ...
              <div className="space-y-4 my-6">
                ...
                <div>
                  ...
                  <div className="w-full bg-light-gray rounded-full h-2 overflow-hidden border border-border">
                    <div className="h-full bg-accent-purple rounded-full transition-all" style={{ width: `${project.progress}%` }}></div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex -space-x-2">
                    {project.team.map((t, i) => (
                      <div key={i} className="w-8 h-8 rounded-full bg-accent-purple text-white flex items-center justify-center font-black text-xs border-2 border-white" title={t}>
                        {t[0]}
                      </div>
                    ))}
                  </div>
            ...
            <div className="pt-4 border-t border-border flex justify-between items-center bg-transparent">
              ...
              <div className="flex gap-2">
                <button onClick={() => updateProgressSimulate(project.id)} className="px-3 py-1.5 bg-accent-purple/10 hover:bg-accent-purple text-accent-purple hover:text-white rounded-lg text-xs font-black transition-all">
                  Progress +10%
                </button>
                <button onClick={() => deleteProject(project.id)} className="p-2 text-text-secondary hover:text-accent-red hover:bg-light-gray rounded-lg transition-all">
                  <X size={16} />
                </button>
              </div>
            </div>

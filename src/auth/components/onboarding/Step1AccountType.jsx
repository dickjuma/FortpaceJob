    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-12">
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-colors ${
              accountType === 'client' ? 'bg-indigo-600 text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
            }`}>
            ...
            <ul className="space-y-3 mt-auto">
              {['Post jobs', 'Hire talent', 'Manage contracts', 'Build teams'].map((feature, i) => (
                <li key={i} className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  <div className={`w-1.5 h-1.5 rounded-full mr-3 ${accountType === 'client' ? 'bg-indigo-500' : 'bg-zinc-300 dark:bg-zinc-600'}`} />
                  {feature}
                </li>
              ))}
            </ul>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-colors ${
              accountType === 'freelancer' ? 'bg-indigo-600 text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
            }`}>
            ...
            <ul className="space-y-3 mt-auto">
              {['Find work', 'Create gigs', 'Get paid', 'Build portfolio'].map((feature, i) => (
                <li key={i} className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  <div className={`w-1.5 h-1.5 rounded-full mr-3 ${accountType === 'freelancer' ? 'bg-indigo-500' : 'bg-zinc-300 dark:bg-zinc-600'}`} />
                  {feature}
                </li>
              ))}
            </ul>

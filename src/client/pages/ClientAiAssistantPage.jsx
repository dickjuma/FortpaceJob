    ...
    <div className="min-h-screen bg-surface dark:bg-surface-dark font-sans pb-24">
      ...
      <div className="bg-white dark:bg-surface-dark border-b border-zinc-200 dark:border-zinc-800 pt-12 pb-8 sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <h1 className="text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight mb-2 flex items-center gap-3">
            <BrainCircuit className="w-8 h-8 text-[#4C1D95]" /> AI Hiring Assistant
          </h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden h-[calc(100vh-12rem)]">
          ...
          <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 bg-[#4C1D95]/5/50 dark:bg-[#4C1D95]/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-[#4C1D95]/10 dark:bg-[#4C1D95]/20 flex items-center justify-center text-[#4C1D95] dark:text-[#4C1D95]">
            ...
            <button className="text-xs font-bold text-zinc-400 hover:text-[#4C1D95] transition-colors">Clear Chat</button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
            {messages.map((msg, i) => (
              ...
              >
                <div className={cn(
                  ...
                  msg.role === 'user' ? "bg-zinc-100 dark:bg-zinc-800 text-zinc-600" : "bg-[#4C1D95] text-white"
                )}>
                  {msg.role === 'user' ? <User className="w-4 h-4" /> : <Sparkles className="w-4 h-4" />}
                </div>
                <div className={cn(
                  "p-4 rounded-2xl text-sm font-medium",
                  msg.role === 'user' ? "bg-[#4C1D95] text-white rounded-tr-sm" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-tl-sm"
                )}>
                  {msg.content}
                </div>
              </motion.div>
            ...
            {isTyping && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-4 max-w-[85%]">
                <div className="w-8 h-8 shrink-0 rounded-full bg-[#4C1D95] text-white flex items-center justify-center">
          ...
          <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 bg-white dark:bg-surface-dark">
            <div className="flex gap-2 mb-3 overflow-x-auto custom-scrollbar pb-1">
              {['Analyze my job post', 'Find UI Designers', 'Optimize my budget', 'Compare candidates'].map(suggestion => (
                <button 
                  ...
                  className="px-3 py-1.5 shrink-0 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-xs font-bold text-zinc-600 dark:text-zinc-300 hover:border-[#4C1D95]/50 hover:text-[#4C1D95] transition-colors"
                >
                  {suggestion}
                </button>
              ))}
            ...
            <form onSubmit={handleSend} className="relative">
              <input 
                ...
                className="w-full bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl pl-4 pr-12 py-4 text-sm font-medium outline-none focus:border-[#4C1D95]/20 shadow-sm"
              />
              <button 
                type="submit"
                disabled={!inputValue.trim() || isTyping}
                className="absolute right-2 top-1/2 -tranzinc-y-1/2 p-2 bg-[#4C1D95] hover:bg-[#22C55E] disabled:opacity-50 text-white rounded-lg transition-colors"
              >
        ...
        <div className="w-full lg:w-[420px] shrink-0 space-y-6">
          ...
          {/* Active Analysis */}
          <div className="bg-gradient-to-br from-[#4C1D95] to-[#22C55E] rounded-3xl p-6 shadow-md text-white">
            <h3 className="font-bold mb-4 flex items-center gap-2"><Target className="w-5 h-5" /> Job Analysis Engine</h3>
            
            <div className="space-y-4">
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                <p className="text-xs font-bold text-[#4C1D95] uppercase tracking-wider mb-1">Optimal Budget</p>
              ...
              <div className="bg-white/10 backdrop-blur-sm rounded-xl p-3 border border-white/20">
                <p className="text-xs font-bold text-[#4C1D95] uppercase tracking-wider mb-1">Time to Hire Forecast</p>
                <div className="flex items-center gap-2">
          ...
          <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#4C1D95]" /> Top Matches
              </h3>
              <span className="text-xs font-bold bg-[#4C1D95]/5 dark:bg-[#4C1D95]/10 text-[#4C1D95] dark:text-[#4C1D95] px-2 py-1 rounded-md">Live Update</span>
            </div>

            <div className="space-y-4">
              {RECOMMENDATIONS.map(freelancer => (
                <div key={freelancer.id} className="p-4 border border-zinc-100 dark:border-zinc-800 rounded-2xl hover:border-[#4C1D95]/20 dark:hover:border-[#4C1D95]/20 transition-colors bg-surface/50 dark:bg-zinc-800/50">
                  ...
                  <div className="bg-[#4C1D95]/5 dark:bg-[#4C1D95]/10 rounded-xl p-3 border border-[#4C1D95]/20 dark:border-[#4C1D95]/20/30 mb-3">
                    <p className="text-xs font-medium text-[#4C1D95] dark:text-[#4C1D95] flex gap-2">
                      <BrainCircuit className="w-4 h-4 shrink-0 text-[#4C1D95]" />
                      {freelancer.reason}
                    </p>
                  </div>

                  <div className="flex gap-2">
                    <button className="flex-1 py-2 bg-[#4C1D95] hover:bg-[#22C55E] text-white text-xs font-bold rounded-lg transition-colors">Invite to Job</button>

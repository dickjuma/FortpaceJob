...
const FILES = [
  { id: 1, name: 'logo-final-v2.svg', type: 'image', size: '1.2 MB', date: 'May 18', uploader: 'Alex R.', version: 'v2', icon: ImageIcon, color: 'text-brand-500', bg: 'bg-brand-50' },
  ...
  { id: 3, name: 'promo-animation.mp4', type: 'video', size: '24.8 MB', date: 'May 16', uploader: 'Alex R.', version: 'v3', icon: Video, color: 'text-brand-500', bg: 'bg-brand-50' },
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <div>
              <div className="flex items-center gap-2 text-xs font-bold text-zinc-500 mb-2">
                <a href="#" className="hover:text-brand-600 transition-colors">Projects</a> <ChevronRight className="w-3 h-3" />
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <button className="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-sm transition-colors flex items-center gap-2">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="relative w-full sm:w-96">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2.5 bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-medium outline-none focus:border-brand-500 transition-colors"
                value={searchQuery}
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto bg-zinc-100 dark:bg-zinc-800 p-1 rounded-xl">
              <button onClick={() => setViewMode('grid')} className={cn("p-2 rounded-lg transition-colors", viewMode === 'grid' ? "bg-white dark:bg-surface-dark shadow-sm text-brand-600" : "text-zinc-500 hover:text-zinc-900 dark:hover:text-white")}>
                <Grid className="w-4 h-4" />
              </button>
              <button onClick={() => setViewMode('list')} className={cn("p-2 rounded-lg transition-colors", viewMode === 'list' ? "bg-white dark:bg-surface-dark shadow-sm text-brand-600" : "text-zinc-500 hover:text-zinc-900 dark:hover:text-white")}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {FOLDERS.map(folder => (
                <div key={folder.id} className="bg-white dark:bg-surface-dark border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 shadow-sm hover:shadow-md hover:border-brand-300 dark:hover:border-brand-700 transition-all cursor-pointer group">
                ...
                {FILES.map(file => (
                  <div key={file.id} className="bg-white dark:bg-surface-dark border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 shadow-sm hover:shadow-md transition-all group relative">
                    ...
                    <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button className="p-1.5 bg-white/90 backdrop-blur-sm rounded-lg shadow-sm text-zinc-700 hover:text-brand-600">
                        <MoreVertical className="w-4 h-4" />
                      </button>
                    </div>
                      ...
                      <tr key={file.id} className="border-b border-zinc-100 dark:border-zinc-800/50 hover:bg-surface dark:hover:bg-zinc-800/20 transition-colors group">
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="p-2 text-zinc-400 hover:text-brand-600 transition-colors rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800"><Download className="w-4 h-4" /></button>
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6">
          ...
          <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
            <h3 className="font-bold text-zinc-900 dark:text-white flex items-center gap-2 mb-6">
              <PieChart className="w-5 h-5 text-brand-500" /> Storage Usage
            </h3>
            ...
            <div className="flex items-center justify-center mb-6">
              <div className="relative w-32 h-32 flex items-center justify-center">
                ...
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle cx="64" cy="64" r="56" className="stroke-brand-500" strokeWidth="12" fill="none" strokeDasharray="351.8" strokeDashoffset="140.7" strokeLinecap="round" />
                </svg>
          ...
          {/* Quick Dropzone */}
          <div className="bg-brand-50 dark:bg-brand-500/10 border-2 border-dashed border-brand-300 dark:border-brand-500/30 rounded-3xl p-8 flex flex-col items-center justify-center text-center cursor-pointer hover:bg-brand-100 dark:hover:bg-brand-500/20 transition-colors group">
            ...
            <h3 className="font-bold text-brand-900 dark:text-brand-400 mb-1">Upload Files</h3>
            <p className="text-xs font-medium text-brand-700 dark:text-brand-300">Drag & drop or click to browse</p>
          </div>

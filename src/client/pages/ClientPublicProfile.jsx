...
import { useClientPublicProfile } from '../services/clientHooks';
import { Building2, Star, CheckCircle2, MapPin, Globe, Clock, ShieldCheck } from 'lucide-react';

const ClientPublicProfile = () => {
  const { id } = useParams();
  const { data: profile, isLoading } = useClientPublicProfile(id);
  
  const fallback = { name: '{current.name}', industry: '{current.industry}', location: 'San Francisco, CA' };
            ...
            <div className="flex flex-col md:flex-row gap-8 items-start">
              ...
              <div className="flex-1">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-4 gap-4">
                  <div>
                    <h1 className="text-3xl font-black text-zinc-900 flex items-center gap-2 mb-1">
                      {current.name} <ShieldCheck className="w-6 h-6 text-success" />
                    </h1>
                    <div className="text-zinc-500 font-medium">{current.industry}</div>
                  </div>
                ...
                <p className="text-zinc-700 leading-relaxed max-w-3xl">
                  {current.name} is a leading provider of global logistics software. We regularly hire top-tier React developers, UI/UX designers, and AWS DevOps engineers to help scale our enterprise platforms. We value clear communication, strict deadlines, and high-quality code.
                </p>
              </div>
            </div>
          </div>

        ...
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight mb-2 flex items-center gap-3">
              <div className="w-10 h-10 bg-[#2bb75c] rounded-xl flex items-center justify-center text-white shadow-sm">
            ...
            {[
              ...
              <button 
                ...
                className={cn(
                  ...
                  activeTab === tab.id ? "border-[#2bb75c]/20 text-[#2bb75c] dark:text-[#2bb75c]" : "border-transparent text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"
                )}
              >
              ...
              <>
                ...
                <div>
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button className="text-sm font-bold text-[#2bb75c] flex items-center gap-1"><Plus className="w-4 h-4" /> New Project</button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {PROJECTS.map(project => (
                      <div key={project.id} className="bg-white dark:bg-surface-dark p-5 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-sm cursor-pointer hover:border-[#2bb75c]/50 transition-colors">
                        <div className="flex justify-between items-start mb-4">
                          <h3 className="font-bold text-zinc-900 dark:text-white">{project.name}</h3>
                          <span className={cn(
                            "text-[10px] font-bold px-2 py-0.5 rounded-md uppercase tracking-wider",
                            project.status === 'In Progress' ? "bg-[#2bb75c]/5 text-[#2bb75c] dark:bg-[#2bb75c]/20 dark:text-[#2bb75c]" : "bg-emerald-50 text-emerald-700 dark:bg-success/20 dark:text-success"
                          )}>{project.status}</span>
                        </div>
                        ...
                        <div className="mb-4">
                          ...
                          <div className="w-full bg-zinc-100 dark:bg-zinc-800 h-1.5 rounded-full overflow-hidden">
                            <div className="h-full bg-[#2bb75c] rounded-full" style={{ width: `${project.progress}%` }}></div>
                          </div>
                        </div>
                ...
                <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-lg font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                      <CheckSquare className="w-5 h-5 text-[#2bb75c]" /> My Tasks
                    </h2>
                      ...
                      <div key={task.id} className="flex items-center justify-between p-3 border border-zinc-100 dark:border-zinc-800 rounded-xl hover:bg-surface dark:hover:bg-zinc-800/50 transition-colors">
                        <div className="flex items-center gap-3">
                          <button className={cn(
                            ...
                            task.completed ? "bg-[#2bb75c] border-[#2bb75c]/20 text-white" : "border-zinc-300 dark:border-zinc-600 bg-transparent"
                          )}>
                    ...
                    <div key={msg.id} className={cn("flex gap-4 max-w-[85%]", msg.isYou ? "ml-auto flex-row-reverse" : "")}>
                      ...
                      <div>
                        <div className={cn("flex items-center gap-2 mb-1", msg.isYou ? "justify-end" : "")}>
                          ...
                          {!msg.isYou && <span className="text-[10px] font-bold text-[#2bb75c] bg-[#2bb75c]/5 dark:bg-[#2bb75c]/10 px-1.5 rounded">{msg.role}</span>}
                          <span className="text-[10px] font-medium text-zinc-400">{msg.time}</span>
                        </div>
                        <div className={cn(
                          "p-3 rounded-2xl text-sm font-medium",
                          msg.isYou ? "bg-[#2bb75c] text-white rounded-tr-sm" : "bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 rounded-tl-sm"
                        )}>
                          {msg.text}
                        </div>
                      </div>
                ...
                <div className="p-4 border-t border-zinc-100 dark:border-zinc-800">
                  <div className="relative">
                    <input 
                      ...
                      className="w-full bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl pl-4 pr-12 py-3 text-sm font-medium outline-none focus:border-[#2bb75c]/20"
                    />
                    <button className="absolute right-2 top-1/2 -tranzinc-y-1/2 p-2 bg-[#2bb75c] hover:bg-[#1d8d38] text-white rounded-lg transition-colors">
            ...
            {(activeTab === 'files') && (
              <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm min-h-[60vh]">
                <div className="flex justify-between items-center mb-8">
                  <h2 className="text-lg font-bold text-zinc-900 dark:text-white flex items-center gap-2">
                    <Folder className="w-5 h-5 text-[#2bb75c]" /> Shared Files
                  </h2>
                  <button className="px-4 py-2 bg-[#2bb75c]/5 dark:bg-[#2bb75c]/10 text-[#2bb75c] dark:text-[#2bb75c] font-bold rounded-lg text-sm flex items-center gap-2">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    { name: 'Brand_Assets', type: 'folder', icon: Folder, color: 'text-[#2bb75c]', bg: 'bg-[#2bb75c]/5 dark:bg-[#2bb75c]/10' }
                  ].map(file => (
                    <div key={file.name} className="border border-zinc-200 dark:border-zinc-800 rounded-2xl p-4 flex flex-col items-center justify-center text-center hover:border-[#2bb75c]/50 transition-colors cursor-pointer group">
            ...
            <div className="space-y-6 hidden lg:block">
              ...
              <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex gap-3">
                    <div className="w-8 h-8 rounded-full bg-[#2bb75c]/10 dark:bg-[#2bb75c]/20 text-[#2bb75c] dark:text-[#2bb75c] flex items-center justify-center shrink-0">
              ...
              {/* Quick Links */}
              <div className="bg-gradient-to-br from-[#2bb75c] to-[#1d8d38] rounded-3xl p-6 text-white shadow-md">

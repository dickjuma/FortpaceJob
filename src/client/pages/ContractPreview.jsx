...
import { useContractDetails, useSignContract } from '../services/clientHooks';

const ContractPreview = () => {
  const { contractId } = useParams();
  const navigate = useNavigate();
  const { data: contract, isLoading } = useContractDetails(contractId || 'fallback');
  const signMutation = useSignContract(contractId);
        ...
        <div className="container mx-auto px-4 md:px-8 max-w-5xl">
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden mb-8 relative">
            ...
            <div className="p-8 border-b border-zinc-200 flex flex-col md:flex-row justify-between gap-8 relative z-10">
              <div>
                <div className="text-sm text-zinc-500 font-medium mb-1">Contract ID: {contract?.id || '#CTR-88492-X9'}</div>
                <h2 className="text-2xl font-bold text-zinc-900">{contract?.job?.title || 'Frontend E-commerce Development'}</h2>
                <div className="mt-4 flex flex-wrap gap-4 text-sm">
                  <div className="bg-surface px-3 py-1.5 rounded-lg border border-zinc-200">
                    <span className="text-zinc-500 mr-1">Type:</span> <span className="font-semibold text-zinc-900">{contract?.type || 'Fixed-Price Milestone'}</span>
                  </div>
                  <div className="bg-surface px-3 py-1.5 rounded-lg border border-zinc-200">
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 p-6 shadow-lg flex flex-col md:flex-row items-center justify-between gap-6 sticky bottom-6 z-50">
            ...
            <div className="flex gap-3 w-full md:w-auto">
              ...
              <button onClick={handleSign} disabled={signMutation.isPending} className="flex-1 md:flex-none flex justify-center items-center gap-2 px-8 py-3 bg-surface-dark hover:bg-zinc-800 text-white font-bold rounded-xl shadow-md transition-colors disabled:opacity-50">
                <FileSignature className="w-5 h-5" /> {signMutation.isPending ? 'Signing...' : 'Sign & Fund Escrow'}
              </button>
            </div>
          </div>

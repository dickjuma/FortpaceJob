...
import { useFreelancerProfile, useClientJobs, useInviteFreelancer } from '../services/clientHooks';
...
const CreateOffer = () => {
  ...
  const { data: profile } = useFreelancerProfile(talentId || 'fallback');
  const { data: jobsData } = useClientJobs();
  const inviteMutation = useInviteFreelancer();
  const [offerType, setOfferType] = useState('milestone');
  
  const talentName = profile?.name || '{talentName}';
  const job = jobsData?.items?.find(j => j.id === jobId) || jobsData?.items?.[0] || { title: 'E-commerce App Frontend Redesign', budgetLabel: 'Fixed-price' };

  const handleSendOffer = async (e) => {
    e.preventDefault();
    if (job?.id) {
      await inviteMutation.mutateAsync({ jobId: job.id, payload: { talentId, offerType } });
    }
    ...
    <>
      <div className="bg-surface min-h-screen py-10">
        <div className="container mx-auto px-4 md:px-8 max-w-6xl">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-zinc-900 mb-2">Create Custom Offer for {talentName}</h1>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="bg-white rounded-2xl p-6 border border-zinc-200 shadow-sm">
                ...
                <div className="p-4 bg-surface rounded-xl border border-zinc-200">
                  <h4 className="font-semibold text-zinc-900 mb-1">{job.title}</h4>
              ...
              <div className="bg-white rounded-2xl p-6 border border-zinc-200 shadow-sm">
                ...
                <div className="flex p-1 bg-zinc-100 rounded-xl mb-6 w-full max-w-md">
                  <button 
                    onClick={(e) => { e.preventDefault(); setOfferType('milestone'); }}
                  ...
                  <button 
                    onClick={(e) => { e.preventDefault(); setOfferType('project'); }}
            ...
            <div className="w-full lg:w-96 flex-shrink-0">
              <div className="bg-surface-dark text-white rounded-2xl p-6 shadow-xl sticky top-24">
                ...
                <button onClick={handleSendOffer} disabled={inviteMutation.isPending} className="w-full flex justify-center items-center gap-2 py-3.5 bg-[#4C1D95] hover:bg-[#22C55E] text-white font-bold rounded-xl transition-colors shadow-lg shadow-blue-900/50 disabled:opacity-50">
                  <Send className="w-4 h-4" /> {inviteMutation.isPending ? 'Sending...' : 'Send Offer'}
                </button>

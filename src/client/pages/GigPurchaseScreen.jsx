          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-6 border-b border-gray-200 dark:border-gray-800">
              <h2 className="text-xl font-bold flex items-center">
                <Package className="w-5 h-5 mr-2 text-brand-600" /> Select Package
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 divide-y md:divide-y-0 md:divide-x divide-gray-200 dark:divide-gray-800">
              {Object.entries(packages).map(([key, pkg]) => (
                <div 
                  ...
                  className={`p-6 cursor-pointer transition-colors ${selectedPackage === key ? 'bg-brand-50 dark:bg-brand-900/20' : 'hover:bg-surface dark:hover:bg-gray-800/50'}`}
                >
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 shadow-sm sticky top-6">
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex justify-between text-gray-600 dark:text-gray-400">
                ...
                <span className="font-medium text-gray-900 dark:text-white">${(packages[selectedPackage].price * 0.05).toLocaleString()}</span>
              </div>
              <div className="pt-4 border-t border-gray-200 dark:border-gray-800 flex justify-between">
                ...
                <span className="font-bold text-lg text-gray-900 dark:text-white">${(packages[selectedPackage].price * 1.05).toLocaleString()}</span>
              </div>
            </div>

            <button className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white rounded-xl font-bold text-lg transition-colors flex justify-center items-center shadow-lg shadow-brand-500/25 mb-4">
              Fund Escrow & Start <ChevronRight className="w-5 h-5 ml-2" />
            </button>

            <div className="flex items-start bg-brand-50 dark:bg-brand-900/20 p-4 rounded-xl border border-brand-100 dark:border-brand-900/50">
              <ShieldCheck className="w-5 h-5 text-brand-600 dark:text-brand-400 mt-0.5 shrink-0 mr-3" />

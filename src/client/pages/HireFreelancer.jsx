...
import { useClientJobs, useFreelancerProfile } from '../services/clientHooks';
...
import { useCheckoutFees } from '../../platform/common/hooks/useCheckoutFees';

const HireFreelancer = () => {
  const { talentId } = useParams();
  const navigate = useNavigate();
  const { requireAuth } = useAuthRedirect();
  const { data: profile, isLoading: isProfileLoading } = useFreelancerProfile(talentId);
  const { data: jobsData, isLoading: isJobsLoading } = useClientJobs();
  
  const fallbackTalent = { id: 'fallback', name: 'Selected Talent', title: 'Freelancer', location: 'Remote', availability: 'Available', hourlyRate: 50 };
  const talent = profile || fallbackTalent;
  const openings = jobsData?.items || [];
              ...
              <div className="bg-white rounded-2xl p-6 border border-zinc-200 shadow-sm">
                ...
                {selectedJob ? (
                  <div className="mt-4 rounded-2xl bg-surface p-4 border border-zinc-200">
                    ...
                    <div className="text-sm text-zinc-600 mt-1">{selectedJob.budgetLabel || `KES ${selectedJob.budgetMin || 0}`} • {selectedJob.location}</div>
                    <div className="flex flex-wrap gap-2 mt-3">
                      {(selectedJob.requiredSkills || selectedJob.skills || []).map((skill) => (

...
import { useClientJobs, useFreelancerProfile, useInviteFreelancer } from '../services/clientHooks';

const InviteFreelancer = () => {
  const { talentId } = useParams();
  const { data: profile } = useFreelancerProfile(talentId);
  const { data: myJobs } = useClientJobs();
  
  const fallbackTalent = { id: 'fallback', name: 'Selected Talent', responseTime: '1 hour', jobSuccess: 95 };
  const talent = profile || fallbackTalent;
  
  const openings = myJobs?.items || [];
          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
            <div className="p-6 bg-surface-dark text-white flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[#4C1D95] text-sm">Responds in about {talent.responseTime}</p>
              </div>
            ...
            <div className="p-6 md:p-8 space-y-8">
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {openings.map((job) => (
                    <div
                      className={`p-4 border rounded-xl cursor-pointer transition-colors flex gap-3 ${selectedJob === job.id ? 'border-[#4C1D95]/20 bg-[#4C1D95]/5/50' : 'border-zinc-200 hover:border-[#4C1D95]/50'}`}
                      key={job.id}
                      onClick={() => setSelectedJob(job.id)}
                    >
                      <Briefcase className={`w-5 h-5 flex-shrink-0 mt-0.5 ${selectedJob === job.id ? 'text-[#4C1D95]' : 'text-zinc-400'}`} />
                      <div>
                        <h4 className="font-semibold text-zinc-900 text-sm mb-1">{job.title}</h4>
                        <p className="text-xs text-zinc-500">{job.budgetLabel || `KES ${job.budgetMin || 0}`}</p>
                      </div>
                    </div>
                  ))}
                  <div
                    className={`p-4 border rounded-xl cursor-pointer transition-colors flex gap-3 ${selectedJob === 'new' ? 'border-[#4C1D95]/20 bg-[#4C1D95]/5/50' : 'border-zinc-200 hover:border-[#4C1D95]/50'}`}
                    onClick={() => setSelectedJob('new')}
                  >
                    <FileText className={`w-5 h-5 flex-shrink-0 mt-0.5 ${selectedJob === 'new' ? 'text-[#4C1D95]' : 'text-zinc-400'}`} />
                    <div>
              ...
              {selectedJob === 'new' ? (
                <div className="bg-surface border border-zinc-200 rounded-xl p-5">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button className="text-xs font-medium text-[#4C1D95] flex items-center gap-1 hover:text-[#4C1D95]" type="button">
                  ...
                  {openings
                    ...
                    .map((job) => (
                      <div key={job.id}>
                        ...
                        <div className="text-sm text-zinc-500 mt-1">{job.budgetLabel || `KES ${job.budgetMin || 0}`} • {job.location}</div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea
                    className="w-full bg-white border border-zinc-300 rounded-xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:border-[#4C1D95]/20 focus:ring-1 focus:ring-[#4C1D95]"
              ...
              <div className="pt-6 border-t border-zinc-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  ...
                  <button className="flex-1 sm:flex-none text-center px-8 py-2.5 bg-[#4C1D95] hover:bg-[#22C55E] text-white font-bold rounded-xl shadow-md transition-colors" type="button">
                    Send invitation
                  </button>
                </div>
              </div>

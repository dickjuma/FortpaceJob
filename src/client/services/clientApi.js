...
export async function getWallet() {
  const r = await apiFetch('/escorow_wallet/wallet');
  return unwrap(r);
}

export async function getTransactions(params = {}) {
  const qs = new URLSearchParams({ limit: 20, ...params }).toString();
  const r = await apiFetch(`/escorow_wallet/wallet/transactions?${qs}`);
  return unwrapPaged(r);
}

export async function getEscrowDetails(contractId) {
  const r = await apiFetch(`/escorow_wallet/escrow/${contractId}`);
  return unwrap(r);
}

export async function fundEscrow(contractId, milestoneId) {
  const r = await apiFetch(`/escorow_wallet/escrow/${contractId}/fund`, {
    method: 'POST',
...
export async function releaseEscrow(contractId, milestoneId) {
  const r = await apiFetch(`/escorow_wallet/escrow/${contractId}/release`, {
    method: 'POST',
    body: JSON.stringify({ milestoneId }),
  });
  return unwrap(r);
...
export async function depositToWallet(payload) {
  const url = payload.provider === 'MPESA' 
    ? '/escorow_wallet/wallet/mpesa/deposit' 
    : '/escorow_wallet/wallet/deposit';
...
export async function withdrawFromWallet(payload) {
  const r = await apiFetch('/escorow_wallet/withdrawal', {
    method: 'POST',
    body: JSON.stringify({
...
export async function getMyProfile() {
  const r = await apiFetch('/profilesystem/profile/me');
  return unwrap(r);
}

export async function updateMyProfile(payload) {
  const r = await apiFetch('/profilesystem/profile/me', { method: 'PATCH', body: JSON.stringify(payload) });
  return unwrap(r);
}
...
export async function approveOrderDelivery(orderId) {
  const r = await apiFetch(`/orders/${orderId}/approve`, { method: 'PATCH' });
  return unwrap(r);
}
...
export async function getMySubscription() {
  const r = await apiFetch('/billing/subscription').catch(() => ({}));
  return unwrap(r) || {};
}

export async function getSubscriptionPlans() {
  const r = await apiFetch('/billing/plans').catch(() => ({ data: [] }));
  return unwrapList(r);
}

export async function upgradeSubscription(plan) {
  const r = await apiFetch('/billing/subscription/upgrade', {
    method: 'POST',

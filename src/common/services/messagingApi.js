/**
 * Unified messaging API — tries canonical /chat/* then legacy /chat_messeging/chat/*
 */
...
export const messagingAPI = {
  getConversations: async (params = {}) => {
    ...
    const data = await tryPaths([
      ...
      `/chat_messeging/chat?${qs}`,
      `/chat_messeging/conversations?${qs}`,
    ]);
  ...
  getMessages: async (conversationId, params = {}) => {
    ...
    const data = await tryPaths([
      ...
      `/chat_messeging/chat/${conversationId}/messages?${qs}`,
    ]);
    return unwrapPaged(data);
  },
  ...
  sendMessage: async (conversationId, content, messageType = 'TEXT') => {
    return tryPaths(
      [
        ...
        `/chat_messeging/chat/${conversationId}/messages`,
      ],
      {
        method: 'POST',
        body: JSON.stringify({ content, messageType, message: content }),
      }
    );
  },

  startConversation: async (payload) => {
    return tryPaths(['/chat/conversations', '/chat_messeging/chat'], {
      method: 'POST',
  ...
  markRead: async (conversationId) => {
    return tryPaths(
      [
        ...
        `/chat_messeging/chat/${conversationId}/read`,
      ],
      { method: 'PATCH' }
    );
  },

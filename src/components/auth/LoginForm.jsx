...
const FloatingInput = ({ label, type = "text", register, error, placeholder, ...rest }) => (
  <div className="relative mb-6">
    <input
      ...
      className={`peer w-full h-16 px-5 pt-5 pb-1 rounded-2xl border-2 ${error ? 'border-red-500 focus:ring-red-500 bg-red-50/50' : 'border-zinc-200 focus:border-[#4C1D95]/20 focus:ring-[#4C1D95] bg-surface hover:bg-zinc-100 focus:bg-white'} text-zinc-900 shadow-sm focus:outline-none focus:ring-1 transition-all placeholder-transparent text-lg`}
      {...register}
      {...rest}
    />
    <label className={`absolute left-5 top-2 text-sm transition-all pointer-events-none ${error ? 'text-red-500' : 'text-zinc-500'} peer-placeholder-shown:text-base peer-placeholder-shown:top-5 peer-focus:top-2 peer-focus:text-sm peer-focus:text-[#4C1D95] font-medium`}>
      {label}
    </label>
    {error && (
    ...
    >
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="relative mb-2">
          ...
          <button 
            type="button" 
            className="absolute right-5 top-5 text-zinc-400 hover:text-[#4C1D95] focus:outline-none transition-colors"
        ...
        <div className="flex items-center justify-between mt-6 mb-8">
          <label className="flex items-center gap-3 cursor-pointer group">
            <div className="relative flex items-center justify-center">
              <input type="checkbox" className="w-5 h-5 rounded border-2 border-zinc-300 text-[#4C1D95] focus:ring-[#4C1D95] cursor-pointer transition-all peer" {...register('rememberMe')} />
            </div>
            <span className="text-zinc-600 font-medium group-hover:text-zinc-900 transition-colors">Remember me</span>
          </label>

          <a href="/auth/forgot-password" className="text-sm font-bold text-[#4C1D95] hover:text-[#4C1D95] transition-colors relative group">
            Forgot Password?
            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#4C1D95] transition-all group-hover:w-full"></span>
          </a>
        </div>
        ...
        <button
          ...
          className="relative w-full h-16 rounded-2xl font-bold text-xl text-white overflow-hidden transition-all duration-300 transform hover:-tranzinc-y-1 shadow-xl hover:shadow-2xl shadow-[#4C1D95]/25 disabled:opacity-80 disabled:cursor-not-allowed disabled:transform-none bg-[#4C1D95] hover:bg-[#22C55E]"
        >
          {isSubmitting ? (

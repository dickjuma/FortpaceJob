    ...
    >
      ...
      <div className="grid md:grid-cols-2 gap-8 mb-12">
        ...
        <motion.div
          ...
          className={`group relative overflow-hidden rounded-[2rem] border-2 cursor-pointer transition-all duration-500 ${
            role === 'client' 
              ? 'border-[#14a800]/20 shadow-2xl shadow-[#14a800]/25' 
              : 'border-zinc-200 shadow-lg shadow-zinc-200/50 hover:border-zinc-300'
          }`}
        >
          {/* Background Image & Overlay */}
          <div className="absolute inset-0">
            ...
            />
            <div className={`absolute inset-0 transition-colors duration-500 ${
              role === 'client' ? 'bg-[#14a800]/40' : 'bg-surface-dark/60'
            }`} />
            <div className="absolute inset-0 bg-gradient-to-t from-zinc-900 via-zinc-900/80 to-transparent" />
          </div>
          ...
          <div className="absolute top-8 right-8 bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-4 w-56 shadow-2xl transform -rotate-2 group-hover:-rotate-4 transition-transform duration-500">
            ...
            <div className="flex -space-x-3">
              ...
              <div className="w-10 h-10 rounded-full border-2 border-zinc-800 bg-[#14a800] flex items-center justify-center text-xs text-white font-bold">+12</div>
            </div>
          </div>
          ...
          <div className="relative z-10 p-10 h-[450px] flex flex-col justify-end">
            {role === 'client' && (
              <motion.div 
                initial={{ scale: 0 }}
                animate={{ scale: 1 }}
                className="absolute top-8 left-8 w-10 h-10 bg-[#14a800] rounded-full flex items-center justify-center shadow-lg"
              >
                <CheckCircle2 className="w-6 h-6 text-white" />
              </motion.div>
            )}
            ...
            <ul className="space-y-2">
              <li className="flex items-center gap-2 text-zinc-400 text-sm"><CheckCircle2 className="w-4 h-4 text-[#14a800]" /> Advanced applicant tracking</li>
              <li className="flex items-center gap-2 text-zinc-400 text-sm"><CheckCircle2 className="w-4 h-4 text-[#14a800]" /> Automated compliance</li>
              <li className="flex items-center gap-2 text-zinc-400 text-sm"><CheckCircle2 className="w-4 h-4 text-[#14a800]" /> Multi-department workflows</li>
            </ul>
          </div>
        </motion.div>

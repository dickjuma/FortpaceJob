...
const FreelancerCard = ({ freelancer }) => {
  ...
  return (
    <article className="flex flex-col h-full bg-white border border-zinc-200 rounded-xl p-5 hover:border-[#4C1D95]/40 hover:shadow-sm transition-all">
      <div className="flex items-start gap-3 mb-3">
        <img src={avatar} alt="" className="w-12 h-12 rounded-full object-cover border border-zinc-100 shrink-0" />
        <div className="min-w-0 flex-1">
          <Link to={profilePath} className="font-bold text-zinc-900 text-sm hover:text-[#4C1D95] line-clamp-1 block">
            {name}
          </Link>
      ...
      <div className="flex flex-wrap items-center gap-2 text-xs text-zinc-500 mb-3">
        {rating != null && (
          <span className="flex items-center gap-1 font-semibold text-zinc-800">
            <Star className="w-3.5 h-3.5 text-[#4C1D95] fill-[#4C1D95]" />
      ...
      <div className="mt-auto pt-3 border-t border-zinc-100 flex items-center justify-between gap-2">
        ...
        <Link
          to={profilePath}
          className="px-3 py-1.5 bg-zinc-900 hover:bg-[#4C1D95] text-white text-xs font-bold rounded-lg transition-colors"
        >
          View
        </Link>
      </div>

            ...
            {step === 1 && (
              <div className="space-y-6">
                  ...
                  <div className="space-y-3">
                    {methods.map(m => (
                      <div 
                        ...
                        className={`p-4 border-2 rounded-xl cursor-pointer flex items-center justify-between transition-colors ${method === m.id ? 'border-brand-600 bg-brand-50 dark:bg-brand-900/20' : 'border-gray-200 dark:border-surface-dark-border hover:border-brand-300'}`}
                      >
                        <div className="flex items-center gap-4">
                          <div className={`p-2 rounded-lg ${method === m.id ? 'bg-brand-600 text-white' : 'bg-gray-100 dark:bg-surface-dark-secondary text-gray-500'}`}>
                ...
                <button 
                  onClick={() => setStep(2)}
                  disabled={!method}
                  className="w-full py-4 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-button font-bold flex justify-center items-center transition-colors shadow-lg shadow-brand-500/25"
                >
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder="0.00"
                      className="w-full pl-9 pr-4 py-4 text-2xl font-bold bg-surface-secondary dark:bg-surface-dark-secondary border-2 border-transparent focus:border-brand-500 rounded-xl text-surface-dark dark:text-white outline-none transition-colors"
                    />
                  </div>
                </div>
                ...
                <div className="bg-surface-tertiary dark:bg-surface-dark-secondary rounded-xl p-4 border border-gray-200 dark:border-surface-dark-border">
                  ...
                  <div className="flex justify-between items-center pt-2 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <span className="font-bold text-brand-600 dark:text-brand-400">${amount || '0.00'}</span>
                  </div>
                </div>
                ...
                <button 
                  ...
                  className="w-full py-4 bg-brand-600 hover:bg-brand-700 disabled:opacity-50 text-white rounded-button font-bold transition-colors shadow-lg shadow-brand-500/25"
                >
                  Confirm & Pay
                </button>
              </div>

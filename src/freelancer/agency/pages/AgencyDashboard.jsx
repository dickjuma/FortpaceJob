...
export default function AgencyDashboard() {
  ...
  const createAgencyMutation = useMutation({
    mutationFn: async (values) => {
      const payload = {
        ...
        ownerId: 'current-user-id', // Handled by backend usually
      };
  ...
  const myAgency = agencies?.[0]; // Support first agency for now
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto space-y-8 font-sans">
      ...
      <Modal title="Create a New Agency" open={createModal} onCancel={() => setCreateModal(false)} footer={null}>
      ...
      <Modal title="Invite Team Member" open={inviteModal} onCancel={() => setInviteModal(false)} footer={null}>
        <Form form={inviteForm} layout="vertical" onFinish={(v) => inviteMutation.mutate({ agencyId: myAgency?._id || myAgency?.id, email: v.email })}>

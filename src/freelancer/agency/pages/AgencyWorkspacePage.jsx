...
import { useAgencyWorkspace } from '../services/agencyHooks';

export default function AgencyWorkspacePage() {
  const { data: response, isLoading } = useAgencyWorkspace();
  const workspaceData = Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [];
  ...
  const initialAnnouncements = workspaceData.length > 0 ? workspaceData : fallbackAnnouncements;

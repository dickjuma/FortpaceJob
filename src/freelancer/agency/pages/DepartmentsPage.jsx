...
import { useAgencyDepartments } from '../services/agencyHooks';

export default function DepartmentsPage() {
  const { data: response, isLoading } = useAgencyDepartments();
  const deptsData = Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [];
  ...
  const departments = deptsData.length > 0 ? deptsData : fallbackDepartments;

...
import { useAgencyFileManager } from '../services/agencyHooks';
...
export default function FileManagerPage() {
  const { data: response, isLoading } = useAgencyFileManager();
  const fileData = Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [];
  const files = fileData.length > 0 ? fileData : INITIAL_FILES;
      ...
      <div className="flex flex-col xl:flex-row items-start xl:items-center justify-between gap-6 border-b border-border pb-6 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <Button 
            ...
            variant="primary" 
            className="bg-success hover:bg-success/95 font-bold text-xs rounded-xl border-none shadow-lg shadow-[#4C1D95]/10"
            icon={<Plus size={16} />}
          >
            Quick Upload
          </Button>

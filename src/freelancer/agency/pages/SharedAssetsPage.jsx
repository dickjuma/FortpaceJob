...
import { useAgencySharedAssets } from '../services/agencyHooks';

export default function SharedAssetsPage() {
  const { data: response, isLoading } = useAgencySharedAssets();
  const assetsData = Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [];
  ...
  const initialAssets = assetsData.length > 0 ? assetsData : fallbackAssets;

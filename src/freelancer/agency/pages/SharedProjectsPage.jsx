...
import { useAgencySharedProjects } from '../services/agencyHooks';

export default function SharedProjectsPage() {
  const { data: response, isLoading } = useAgencySharedProjects();
  const projectsData = Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [];
  
  // Use mock data as fallback during loading/empty state for now
  ...
  // Sync projects state when query data changes
  React.useEffect(() => { if (projectsData.length > 0) setProjects(projectsData); else setProjects(fallbackProjects); }, [projectsData]);
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans animate-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        {[
          ...
          { label: 'Average Progress', value: `${Math.round(projects.reduce((acc, p) => acc + p.progress, 0) / projects.length)}%`, icon: Clock, color: 'text-[#4C1D95]', bg: 'bg-[#4C1D95]/5' },

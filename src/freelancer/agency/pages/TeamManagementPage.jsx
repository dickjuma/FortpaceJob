...
import {
  Users, Mail, CheckCircle, Clock, Shield, Search,
...
export default function TeamManagementPage() {
  ...
  const { data: page = {}, isLoading, error, refetch } = useAgencyTeam();
  const members = page.items ?? [];

  const inviteMutation = useInviteAgencyMember();
  const removeMutation = useRemoveAgencyMember();
  const roleUpdateMutation = useUpdateAgencyMemberRole();
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white flex items-center gap-2">
              <Users className="w-6 h-6 text-[#4C1D95]" /> Team Management
            </h1>
            <p className="text-sm text-zinc-400 mt-1">Invite and manage agency team members</p>
          </div>
          <button
            onClick={() => setShowInviteModal(true)}
            className="flex items-center gap-2 px-5 py-2.5 bg-[#4C1D95] hover:bg-[#22C55E] text-white rounded-full text-sm font-bold transition-all"
          >
        ...
        <div className="flex gap-3 items-center">
          <div className="relative flex-1 max-w-sm">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-700 rounded-xl pl-9 pr-4 py-2 text-sm text-zinc-200 placeholder-zinc-500 focus:outline-none focus:border-[#4C1D95]"
            />
          </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 gap-3 bg-zinc-900/30 rounded-2xl border border-zinc-800">
            ...
            <button onClick={refetch} className="text-xs text-[#4C1D95] hover:underline">Retry</button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-48 gap-4 bg-zinc-900/30 rounded-2xl border border-zinc-800">
            ...
            <button onClick={() => setShowInviteModal(true)} className="px-5 py-2 bg-[#4C1D95] text-white rounded-full text-sm font-bold">
              Invite First Member
            </button>
          </div>
        ) : (
                    ...
                    <tr key={member.id} className="hover:bg-zinc-800/30 transition-colors">
                      ...
                      <td className="px-5 py-4">
                        {editingRole === member.id && !isOwner ? (
                          <select
                            defaultValue={role}
                            onChange={(e) => roleUpdateMutation.mutate(
                              { memberId: member.id, payload: { role: e.target.value } },
                              {
                                onSuccess: () => {
                      ...
                      <td className="px-5 py-4">
                        {status === 'Active' || status === 'ACTIVE' ? (
                          <span className="flex items-center gap-1 text-[#4C1D95] font-bold text-xs">
          ...
          {ROLES.map((role) => (
            <div key={role.name} className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <Shield className="w-5 h-5 text-[#4C1D95]" />
              ...
              <div className="space-y-1.5">
                {role.permissions.map((perm) => (
                  <div key={perm} className="flex items-center gap-2 text-xs text-zinc-400">
                    <Briefcase className="w-3 h-3 text-[#4C1D95]" /> {perm}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button
                ...
                className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-[#4C1D95] text-white rounded-xl text-sm font-bold disabled:opacity-50"
              >

...
import { useFreelancerDashboard } from '../services/freelancerHooks';

const FreelancerDashboard = () => {
  const { data: response, isLoading } = useFreelancerDashboard();
  const dashboardData = Array.isArray(response?.data) ? response.data : Array.isArray(response) ? response : [];
  ...
  const earningsData = dashboardData?.earningsData?.length > 0 ? dashboardData.earningsData : fallbackEarningsData;
  const proposalData = dashboardData?.proposalData?.length > 0 ? dashboardData.proposalData : fallbackProposalData;
  const recommendedJobs = dashboardData?.recommendedJobs?.length > 0 ? dashboardData.recommendedJobs : fallbackRecommendedJobs;
  return (

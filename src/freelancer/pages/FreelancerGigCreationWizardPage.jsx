      ...
      <div className="bg-white dark:bg-surface-dark border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-40">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6">
          ...
          <div className="py-6 flex justify-between relative">
            ...
            <div className="absolute top-1/2 left-0 h-1 bg-brand-500 -tranzinc-y-1/2 z-0 transition-all duration-500" style={{ width: `${((currentStep - 1) / (STEPS.length - 1)) * 100}%` }}></div>
              ...
              return (
                <div key={step.id} className="relative z-10 flex flex-col items-center gap-2">
                  <div className={cn(
                    ...
                    isActive ? "bg-white dark:bg-surface-dark border-brand-500 text-brand-600 dark:text-brand-400 shadow-lg" : 
                  ...
                  <span className={cn("text-xs font-bold uppercase tracking-wider absolute -bottom-6 w-32 text-center", isActive ? "text-brand-600 dark:text-brand-400" : isCompleted ? "text-zinc-900 dark:text-white" : "text-zinc-400")}>
                    {step.name}
                  </span>
                </div>
              );
      ...
      <div className="flex-1 max-w-[1400px] w-full mx-auto px-4 sm:px-6 py-12 flex gap-8">
        ...
        <div className="flex-1 max-w-4xl">
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="flex relative">
                      ...
                      <textarea 
                        className="w-full pl-16 pr-4 py-4 bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-lg font-medium text-zinc-900 dark:text-white focus:border-brand-500 outline-none resize-none"
                        rows="2"
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    <div>
                      ...
                      <select className="w-full px-4 py-3 bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-medium outline-none focus:border-brand-500">
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full px-4 py-3 bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-medium outline-none focus:border-brand-500"
                    />
                  </div>
                </div>
              </motion.div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {[1,2,3].map(i => (
                        <div key={i} className="aspect-video bg-surface dark:bg-zinc-800 border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-700/50 transition-colors group">
                          <UploadCloud className="w-8 h-8 text-zinc-400 group-hover:text-brand-500 transition-colors mb-2" />
                          <span className="text-xs font-bold text-zinc-500 group-hover:text-brand-600">Drag & drop a Photo</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="aspect-video md:w-2/3 bg-surface dark:bg-zinc-800 border-2 border-dashed border-zinc-300 dark:border-zinc-700 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-700/50 transition-colors group">
                      <UploadCloud className="w-8 h-8 text-zinc-400 group-hover:text-brand-500 transition-colors mb-2" />
                      <span className="text-xs font-bold text-zinc-500 group-hover:text-brand-600">Drag & drop a Video</span>
                    </div>
                  </div>
                </div>
              </motion.div>
          ...
          <div className="mt-8 flex justify-between items-center">
            ...
            <button 
              onClick={nextStep}
              className="px-8 py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-lg shadow-brand-600/20 transition-all flex items-center gap-2"
            >
        ...
        <div className="w-80 shrink-0 hidden lg:block space-y-6">
          <div className="bg-gradient-to-br from-brand-600 to-indigo-600 rounded-3xl p-6 shadow-md text-white">
            ...
            <p className="text-sm font-medium text-brand-100 mb-4 leading-relaxed">Need help writing your title? Provide a brief description of what you do, and I'll suggest SEO-optimized titles.</p>
            <textarea 
              className="w-full bg-white/10 border border-white/20 rounded-xl p-3 text-sm font-medium placeholder:text-brand-200 outline-none focus:border-white mb-3"
              rows="3"
              placeholder="E.g. I build react websites..."
            />
            <button className="w-full py-2 bg-white text-brand-600 font-bold rounded-lg shadow-sm">Generate Titles</button>
          </div>

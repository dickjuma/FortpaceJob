...
const CERTIFICATIONS = [
  { id: 1, name: 'React Native Expert', date: 'May 10, 2026', score: '98%', icon: Award, color: 'text-brand-500', bg: 'bg-brand-50 dark:bg-brand-500/10' },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight mb-2 flex items-center gap-3">
              <GraduationCap className="w-8 h-8 text-brand-500" /> Learning & Certifications
            </h1>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="bg-gradient-to-br from-brand-600 to-indigo-600 rounded-3xl p-6 shadow-md text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <p className="text-xs font-bold text-brand-200 uppercase tracking-wider mb-2">Learning Score</p>
                  <h3 className="text-4xl font-black mb-1">Top 5%</h3>
                  <p className="text-sm font-medium text-brand-100">You learn faster than 95% of freelancers.</p>
                </div>
              </div>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {COURSES.map(course => (
                  <div key={course.id} className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 shadow-sm overflow-hidden group cursor-pointer hover:border-brand-300 transition-colors flex flex-col">
                    ...
                    <div className="p-6 flex-1 flex flex-col">
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-brand-600 bg-brand-50 dark:bg-brand-500/10 px-2 py-0.5 rounded-md">
                          {course.level}
                        </span>
                      ...
                      <div className="mt-auto">
                        ...
                        <div className="w-full bg-zinc-100 dark:bg-zinc-800 h-1.5 rounded-full overflow-hidden">
                          <div className={cn("h-full rounded-full transition-all duration-1000", course.progress === 100 ? "bg-success" : "bg-brand-500")} style={{ width: `${course.progress}%` }}></div>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                {[
                  ...
                  <div key={skill.name} className="p-4 border border-zinc-100 dark:border-zinc-800 rounded-2xl hover:bg-surface dark:hover:bg-zinc-800/50 transition-colors flex justify-between items-center cursor-pointer">
                    ...
                    <div className="text-right">
                      ...
                      <button className="mt-2 text-sm font-bold text-brand-600 flex items-center gap-1 hover:underline">
        ...
        {activeTab === 'certifications' && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {/* Add New */}
              <div className="border-2 border-dashed border-zinc-200 dark:border-zinc-700 rounded-3xl p-6 flex flex-col items-center justify-center min-h-[200px] hover:border-brand-500 hover:bg-brand-50 dark:hover:bg-brand-500/5 cursor-pointer transition-all text-center">

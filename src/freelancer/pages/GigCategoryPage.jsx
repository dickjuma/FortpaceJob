      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 w-full space-y-16">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {SUBCATEGORIES.map((sub, i) => (
              <motion.div 
                ...
                key={sub.name}
                className="bg-white dark:bg-surface-dark border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 text-center cursor-pointer group hover:border-brand-500 hover:shadow-lg hover:shadow-brand-500/10 transition-all"
              >
                <div className="w-12 h-12 bg-surface dark:bg-zinc-800 rounded-xl flex items-center justify-center mx-auto mb-4 group-hover:bg-brand-50 dark:group-hover:bg-brand-500/10 transition-colors">
                  <sub.icon className="w-6 h-6 text-zinc-400 group-hover:text-brand-500 transition-colors" />
                </div>
                <h3 className="text-sm font-bold text-zinc-900 dark:text-white mb-1 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{sub.name}</h3>
        ...
        <div>
          <div className="flex justify-between items-end mb-6">
            ...
            <button className="hidden sm:flex items-center gap-1 text-sm font-bold text-brand-600 hover:text-brand-700 transition-colors">
            ...
            {POPULAR_GIGS.map(gig => (
              <div key={gig.id} className="bg-white dark:bg-surface-dark rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden group hover:shadow-xl hover:-tranzinc-y-1 transition-all cursor-pointer flex flex-col">
                ...
                <div className="p-4 flex-1 flex flex-col">
                  ...
                  <h3 className="text-sm font-bold text-zinc-700 dark:text-zinc-300 leading-snug mb-3 line-clamp-2 hover:text-brand-600">
                    {gig.title}
                  </h3>
        ...
        <div className="bg-zinc-100 dark:bg-surface-dark rounded-3xl p-8 border border-zinc-200 dark:border-zinc-800">
          ...
          <div className="flex flex-wrap gap-2">
            {['React JS', 'Python', 'WordPress', 'Shopify', 'iOS Apps', 'Android', 'Machine Learning'].map(tag => (
              <span key={tag} className="px-3 py-1.5 bg-white dark:bg-zinc-800 rounded-lg text-xs font-bold text-zinc-500 border border-zinc-200 dark:border-zinc-700 cursor-pointer hover:border-brand-500 transition-colors">
                {tag}
              </span>
            ))}
          </div>

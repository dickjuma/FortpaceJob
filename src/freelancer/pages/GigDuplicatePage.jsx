    ...
    <div className="min-h-screen bg-surface dark:bg-surface-dark font-sans flex items-center justify-center py-12 px-4 sm:px-6">
      ...
      <div className="max-w-2xl w-full">
        ...
        <div className="text-center mb-8">
          <div className="w-16 h-16 bg-brand-100 dark:bg-brand-500/20 text-brand-600 dark:text-brand-400 rounded-2xl flex items-center justify-center mx-auto mb-4">
          ...
          {isDone ? (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center py-8">
              ...
              <div className="flex items-center justify-center gap-4">
                <button className="px-6 py-2.5 bg-brand-600 text-white text-sm font-bold rounded-xl shadow-sm hover:bg-brand-700 transition-colors">
                  Edit New Gig
                </button>
            ...
            <div className="space-y-8">
              ...
              <div>
                ...
                <input 
                  ...
                  className="w-full p-4 bg-white dark:bg-surface-dark border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-bold text-zinc-900 dark:text-white outline-none focus:ring-2 focus:ring-brand-500 transition-all"
                />
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <button onClick={() => setSelectedOptions(CLONE_OPTIONS.map(o => o.id))} className="text-xs font-bold text-brand-600 hover:text-brand-700">Select All</button>
                </div>
                    ...
                    return (
                      <div 
                        ...
                        className={cn(
                          ...
                          isSelected ? "border-brand-500 bg-brand-50 dark:bg-brand-500/10" : "border-zinc-200 dark:border-zinc-700 bg-white dark:bg-surface-dark hover:border-zinc-300 dark:hover:border-zinc-600"
                        )}
                      >
                        <div className={cn(
                          "w-5 h-5 rounded border flex items-center justify-center transition-colors shrink-0",
                          isSelected ? "bg-brand-500 border-brand-500" : "border-zinc-300 dark:border-zinc-600 bg-white dark:bg-zinc-800"
                        )}>
                          {isSelected && <Check className="w-3.5 h-3.5 text-white" />}
                        </div>
                        <option.icon className={cn("w-5 h-5", isSelected ? "text-brand-500" : "text-zinc-400")} />
                        <span className={cn("text-sm font-bold", isSelected ? "text-brand-700 dark:text-brand-400" : "text-zinc-600 dark:text-zinc-400")}>
                          {option.label}
                        </span>
                      </div>
                    );

        ...
        <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm flex items-center justify-between">
          ...
          <div 
            ...
            className={cn(
              ...
              packagesEnabled ? "bg-brand-500" : "bg-zinc-200 dark:bg-zinc-700"
            )}
          >
          ...
          <div className="grid grid-cols-4 border-b border-zinc-200 dark:border-zinc-800 bg-surface dark:bg-zinc-800/30">
            ...
            {['basic', 'standard', 'premium'].map((pkgType, idx) => (
              <div key={pkgType} className={cn(
                ...
                pkgType === 'standard' && packagesEnabled ? "bg-brand-50/50 dark:bg-brand-500/5" : "",
                !packagesEnabled && pkgType !== 'basic' ? "opacity-30 pointer-events-none grayscale" : ""
              )}>
                {pkgType === 'standard' && packagesEnabled && (
                  <div className="absolute top-0 left-1/2 -tranzinc-x-1/2 bg-brand-500 text-white text-[10px] font-black uppercase tracking-wider px-3 py-1 rounded-b-lg">
                    Recommended
                  </div>
                )}
                <h4 className={cn(
                  "text-lg font-black uppercase tracking-wide mb-1",
                  pkgType === 'basic' ? "text-zinc-700 dark:text-zinc-300" : pkgType === 'standard' ? "text-brand-600 dark:text-brand-400" : "text-violet-600 dark:text-violet-400"
                )}>
          ...
          <div className="grid grid-cols-4 border-b border-zinc-200 dark:border-zinc-800">
            ...
            {['basic', 'standard', 'premium'].map((pkgType, idx) => (
              <div key={pkgType} className={cn(
                ...
                idx !== 2 && "border-r border-zinc-200 dark:border-zinc-800",
                pkgType === 'standard' && packagesEnabled ? "bg-brand-50/30 dark:bg-brand-500/5" : "",
                ...
                <input 
                  ...
                  className="w-full text-sm font-bold bg-zinc-100 dark:bg-zinc-800 px-3 py-2 rounded-lg outline-none focus:ring-2 focus:ring-brand-500 text-zinc-900 dark:text-white placeholder:text-zinc-400"
                />
                <textarea 
                  rows="4"
                  ...
                  className="w-full text-xs font-medium bg-zinc-100 dark:bg-zinc-800 px-3 py-2 rounded-lg outline-none focus:ring-2 focus:ring-brand-500 resize-none text-zinc-700 dark:text-zinc-300 placeholder:text-zinc-400"
                />
              </div>
            ))}
          </div>
                ...
                {packages[pkgType].features.map((feature, fIdx) => (
                  <div key={fIdx} className={cn("px-6 py-4 flex items-center justify-center h-14", fIdx !== packages[pkgType].features.length -1 && "border-b border-zinc-100 dark:border-zinc-800/50")}>
                    <input 
                      ...
                      className="w-5 h-5 text-brand-600 rounded border-zinc-300 focus:ring-brand-500 cursor-pointer"
                    />
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-4 bg-surface dark:bg-zinc-800/30">
            ...
            {['basic', 'standard', 'premium'].map((pkgType, idx) => (
              <div key={pkgType} className={cn(
                ...
                idx !== 2 && "border-r border-zinc-200 dark:border-zinc-800",
                pkgType === 'standard' && packagesEnabled ? "bg-brand-50/50 dark:bg-brand-500/10" : "",
                !packagesEnabled && pkgType !== 'basic' ? "opacity-30 pointer-events-none" : ""
              )}>
                <select 
                  value={packages[pkgType].delivery}
                  onChange={(e) => handlePackageChange(pkgType, 'delivery', parseInt(e.target.value))}
                  className="w-full p-2 bg-white dark:bg-surface-dark border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm font-bold text-zinc-900 dark:text-white outline-none focus:border-brand-500"
                >
                ...
                <select 
                  ...
                  className="w-full p-2 bg-white dark:bg-surface-dark border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm font-bold text-zinc-900 dark:text-white outline-none focus:border-brand-500 mt-2"
                >
              ...
              {ADDON_SUGGESTIONS.map(sug => {
                ...
                return (
                  <button 
                    ...
                    disabled={isAdded}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-bold border transition-all disabled:opacity-50 disabled:cursor-not-allowed border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:border-brand-500 hover:text-brand-600"
                  >
      ...
      <div className="w-full lg:w-80 shrink-0 space-y-6">
        ...
        <div className="bg-gradient-to-br from-brand-600 to-violet-600 rounded-3xl p-6 text-white shadow-xl shadow-brand-500/20">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-brand-50">Pricing Insights</h3>
          </div>
          
          <h4 className="text-3xl font-black mb-1">KES 8,500</h4>
          <p className="text-sm text-brand-100 font-semibold mb-6">Average order value in your category</p>
          ...
          <div className="space-y-4">
            <div className="bg-white/10 p-3 rounded-xl border border-white/10 flex items-start gap-3">
              ...
              <p className="text-xs leading-relaxed text-brand-50 font-medium">Your <span className="font-bold text-white">Basic (KES 5000)</span> package is competitively priced to attract new buyers.</p>
            </div>
            <div className="bg-white/10 p-3 rounded-xl border border-white/10 flex items-start gap-3">
              <TrendingUp className="w-4 h-4 text-amber-300 shrink-0 mt-0.5" />
              <p className="text-xs leading-relaxed text-brand-50 font-medium">Consider raising your <span className="font-bold text-white">Premium (KES 25000)</span> package. Top sellers average KES 40000 for full solutions.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
              <BarChart3 className="w-4 h-4 text-brand-500" /> Market Standard
            </h3>
        ...
        <div className="bg-brand-50 dark:bg-brand-900/10 rounded-3xl border border-brand-100 dark:border-brand-900/30 p-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <h3 className="text-sm font-bold text-brand-900 dark:text-brand-400">Upselling Strategy</h3>
          </div>

        ...
        <div className="space-y-4">
          ...
          {requirements.length === 0 && !isAdding && (
            <div className="text-center py-12 border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-3xl bg-surface dark:bg-surface-dark/50">
              ...
              <button onClick={() => setIsAdding(true)} className="px-6 py-2.5 bg-brand-600 text-white font-bold text-sm rounded-xl">Add First Requirement</button>
            </div>
          )}
          ...
          <AnimatePresence>
            {isAdding && (
              <motion.div 
                ...
                className="bg-white dark:bg-surface-dark border-2 border-brand-500 shadow-lg shadow-brand-500/10 rounded-2xl p-6 relative"
              >
                ...
                <div className="space-y-6">
                      ...
                      {REQUIREMENT_TYPES.map(type => (
                        <button
                          ...
                          className={cn(
                            ...
                            newReqType === type.id ? "bg-brand-50 dark:bg-brand-500/10 border-brand-500 text-brand-700 dark:text-brand-400" : "bg-surface dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-400 hover:border-zinc-300"
                          )}
                        >
                          <type.icon className={cn("w-5 h-5 mb-1.5", newReqType === type.id ? "text-brand-500" : "text-zinc-400")} />
                  ...
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full p-4 bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm font-bold text-zinc-900 dark:text-white outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all"
                    />
                  </div>
                  ...
                  {(newReqType === 'multiple' || newReqType === 'checklist') && (
                    <div>
                        ...
                        {newOptions.map((opt, i) => (
                          <div key={i} className="flex items-center gap-2">
                            <input 
                              ...
                              className="flex-1 p-2 bg-surface dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm font-medium outline-none focus:border-brand-500"
                            />
                      ...
                      <button onClick={() => setNewOptions([...newOptions, `Option ${newOptions.length + 1}`])} className="text-xs font-bold text-brand-600 flex items-center gap-1 hover:underline">
                  ...
                  <div className="flex items-center justify-between pt-4 border-t border-zinc-100 dark:border-zinc-800">
                    <label className="flex items-center gap-2 cursor-pointer">
                      <div className={cn(
                        ...
                        newRequired ? "bg-brand-500" : "bg-zinc-200 dark:bg-zinc-700"
                      )}>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={addRequirement} disabled={!newQuestion} className="px-6 py-2 bg-brand-600 disabled:bg-zinc-300 text-white text-sm font-bold rounded-xl transition-colors">Add</button>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      <div className="w-full lg:w-80 shrink-0 space-y-6">
        ...
        <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {TEMPLATES.map(temp => (
              <div key={temp.id} className="p-3 bg-surface dark:bg-zinc-800/50 rounded-xl border border-zinc-100 dark:border-zinc-700/50 group cursor-pointer hover:border-brand-300 transition-colors" onClick={loadTemplate}>
                <div className="flex justify-between items-start mb-1">
                  <h4 className="text-xs font-bold text-zinc-900 dark:text-white">{temp.title}</h4>
                  <Plus className="w-3.5 h-3.5 text-zinc-400 group-hover:text-brand-500 transition-colors" />
                </div>
        ...
        <div className="bg-gradient-to-br from-brand-600 to-violet-600 rounded-3xl p-6 text-white shadow-xl shadow-brand-500/20">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="font-bold text-brand-50">Conversion Tip</h3>
          </div>

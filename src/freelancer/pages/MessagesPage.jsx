import React, { useEffect, useState, useCallback } from 'react';
import { Link } from 'react-router-dom';
import {
  useFreelancerChats,
  useFreelancerMessages,
  useSendFreelancerMessage,
} from '../services/freelancerHooks';
import { useAuthStore } from '../../common/authStore';
import { messagingAPI } from '../../common/services/messagingApi';
import MessagesInbox from '../../components/messaging/MessagesInbox';
import VideoCallModal from '../../components/ui/VideoCallModal';
import { useMessagingSocket } from '../../common/hooks/useMessagingSocket';
import { useSocket } from '../../common/context/SocketContext';

export default function MessagesPage() {
  const { user } = useAuthStore();
  const [activeId, setActiveId] = useState(null);
  const [localMessages, setLocalMessages] = useState([]);
  const [videoOpen, setVideoOpen] = useState(false);
  const { isConnected } = useSocket();

  const { data: chatsData, isLoading, error, refetch } = useFreelancerChats({ limit: 50 });
  const { data: messagesData, isLoading: msgsLoading, refetch: refetchMsgs } = useFreelancerMessages(activeId);
  const sendMutation = useSendFreelancerMessage();

  const conversations = chatsData?.items || [];
  const apiMessages = messagesData?.items || [];

  useEffect(() => {
    setLocalMessages(apiMessages);
  }, [apiMessages]);

  useEffect(() => {
    if (!activeId && conversations.length > 0) {
      setActiveId(conversations[0].id);
    }
  }, [conversations, activeId]);

  useEffect(() => {
    if (!activeId) return undefined;
    messagingAPI.markRead(activeId).catch(() => {});
    const timer = setInterval(() => {
      refetchMsgs();
      refetch();
    }, 12000);
    return () => clearInterval(timer);
  }, [activeId, refetchMsgs, refetch]);

  const onIncomingMessage = useCallback(
    (message) => {
      if (!message?.id) return;
      setLocalMessages((prev) => (prev.some((m) => m.id === message.id) ? prev : [...prev, message]));
      refetch();
    },
    [refetch]
  );

  useMessagingSocket({ conversationId: activeId, onIncomingMessage });

  const selected = conversations.find((c) => c.id === activeId);
  const other = selected?.otherParticipant || selected?.participant || {};

  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      {!isConnected && (
        <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-3">
          Real-time connection offline — messages still sync via API.
        </p>
      )}
      <MessagesInbox
        backLink={
          <Link to="/freelancer/dashboard" className="text-sm font-medium text-[#14a800] hover:underline">
            ← Dashboard
          </Link>
        }
        conversations={conversations}
        conversationsLoading={isLoading}
        conversationsError={error?.message}
        onRefreshConversations={refetch}
        messages={localMessages}
        messagesLoading={msgsLoading}
        selectedId={activeId}
        onSelectConversation={setActiveId}
        onSendMessage={async (text) => {
          await sendMutation.mutateAsync({ conversationId: activeId, content: text });
          refetchMsgs();
          refetch();
        }}
        sending={sendMutation.isPending}
        currentUserId={user?.id}
        emptyTitle="Messages from clients will appear here."
        videoEnabled={Boolean(other?.id)}
        onStartVideoCall={other?.id ? () => setVideoOpen(true) : undefined}
      />
      {videoOpen && other?.id && (
        <VideoCallModal
          isOpen={videoOpen}
          onClose={() => setVideoOpen(false)}
          isIncoming={false}
          callerName={other.name}
          callerId={user?.id}
          receiverId={other.id}
          conversationId={activeId}
          callType="VIDEO"
        />
      )}
    </div>
  );
}

...
const CATEGORY_COLORS = {
  Development: 'bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300',
  Design: 'bg-brand-100 dark:bg-brand-900/30 text-brand-700 dark:text-brand-300',
    ...
    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
      ...
      <div className="bg-gradient-to-r from-brand-600 to-violet-600 p-6 text-white">
        <div className="flex justify-between items-start">
          <div>
            ...
            <p className="text-brand-100 text-sm mt-0.5">#{String(project.id).padStart(5, '0')}</p>
          </div>
        ...
        <div className="mt-4 flex justify-between items-end">
          <div>
            <p className="text-brand-100 text-xs">Billed to</p>
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex justify-between items-center border-t border-gray-100 dark:border-gray-800 pt-3">
          ...
          <span className="text-xl font-bold text-brand-600">${(project.budget * 0.95).toLocaleString()}</span>
        </div>
  ...
  <motion.div layout
    ...
    <div className="p-5" onClick={() => onExpand(project.id)}>
      <div className="flex items-start justify-between gap-2 mb-3">
        <h3 className="font-semibold text-gray-900 dark:text-white text-sm leading-snug group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{project.title}</h3>
  ...
  <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.04 }}
    ...
    <div className="flex items-center gap-4 px-5 py-3.5 hover:bg-surface dark:hover:bg-gray-800/50 transition-colors cursor-pointer" onClick={() => onExpand(project.id)}>
      ...
      <button onClick={e => { e.stopPropagation(); onInvoice(project); }}
        ...
        <Download size={14} className="text-gray-400 hover:text-brand-600" />
      </button>
        ...
        <div key={month} className="mb-8">
          <div className="sticky top-16 z-10 flex items-center gap-3 mb-4 bg-surface dark:bg-gray-950 py-1">
            <div className="w-12 flex justify-center z-10">
              <div className="w-3 h-3 rounded-full bg-brand-600 ring-4 ring-white dark:ring-gray-950" />
            </div>
            <span className="text-sm font-semibold text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-900/20 px-3 py-1 rounded-full">{month}</span>
          </div>
...
export default function WorkHistoryPage() {
  ...
  const SortIcon = ({ col }) => {
    ...
    return sortDir === 'desc' ? <ChevronDown size={12} className="text-brand-600" /> : <ChevronUp size={12} className="text-brand-600" />;
  };
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans min-h-screen bg-surface dark:bg-gray-950">
          ...
          <div className="flex bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-1 gap-1">
            {[
              ...
              <button key={v} onClick={() => setView(v)}
                className={`p-2 rounded-lg transition-all duration-200 ${view === v ? 'bg-brand-600 text-white shadow-sm' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                <Icon size={16} />
              </button>
            ))}
          </div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show"
        ...
        {[
          { label: 'Total Projects', value: '47', icon: FileText, color: 'text-brand-600', bg: 'bg-brand-50 dark:bg-brand-900/20' },
          ...
          { label: 'Repeat Clients', value: '62%', icon: Users, color: 'text-brand-600', bg: 'bg-brand-50 dark:bg-brand-900/20' },
        ].map((stat) => {
        ...
        <div className="flex gap-3 flex-wrap items-center">
          <div className="relative flex-1 min-w-48">
            ...
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search projects..."
              className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500" />
          </div>
          <button onClick={() => setShowFilters(v => !v)}
            className={`flex items-center gap-2 px-4 py-2.5 border rounded-xl text-sm font-medium transition-colors ${showFilters ? 'bg-brand-600 border-brand-600 text-white' : 'bg-white dark:bg-gray-900 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:border-brand-400'}`}>
          ...
          <div className="flex gap-1.5 flex-wrap">
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setCategory(cat)}
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${category === cat ? 'bg-brand-600 text-white border-brand-600' : 'bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:border-brand-400'}`}>
                {cat}
              </button>
            ))}
          </div>
              ...
              <div className="mt-3 p-5 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                <div>
                  ...
                  <input value={clientSearch} onChange={e => setClientSearch(e.target.value)} placeholder="Search client..."
                    className="w-full px-3 py-2 bg-surface dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500 text-gray-900 dark:text-white" />
                </div>
                <div>
      ...
      <AnimatePresence mode="wait">
        {filtered.length === 0 ? (
          ...
          <motion.div key="empty" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            ...
            <button onClick={clearFilters}
              className="px-5 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-lg text-sm font-medium transition-colors flex items-center gap-2">

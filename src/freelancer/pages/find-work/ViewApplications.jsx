...
import { useProposalsForJob as useJobProposals, useShortlistProposal, useRejectProposal } from '../../../client/services/clientHooks';
...
const ViewApplications = () => {
  ...
  const { data: job } = useFindWorkJob(workId);
  const { data: proposalsResponse, isLoading, refetch } = useJobProposals(workId);
  const { mutateAsync: shortlistProposal } = useShortlistProposal();
  const { mutateAsync: rejectProposal } = useRejectProposal();

  const jobTitle = job?.title || '';

  const applications = useMemo(() => {
    return extractList(proposalsResponse).map(p => mapProposalToCard(p, {}));
  }, [proposalsResponse]);

  useEffect(() => {
    syncJobsWithBackend({ limit: 100 });
  }, []);

...
import { useWithdrawProposal } from '../../services/freelancerHooks';
import { getProviderApplications, syncApplicationsWithBackend } from './findWorkData';

const WithdrawApplication = () => {
  const { appId } = useParams();
  const navigate = useNavigate();
  const { mutateAsync: withdrawProposal } = useWithdrawProposal();
    ...
    <>
      <div className="bg-surface min-h-screen py-10">
        <div className="container mx-auto px-4 md:px-8 max-w-3xl">
          ...
          <Link to="/find-work/my-applications" className="inline-flex items-center gap-2 text-sm font-bold text-zinc-500 hover:text-[#4C1D95] transition-colors mb-6">
              ...
              <div className="bg-white border border-zinc-200 rounded-3xl p-6 md:p-8 shadow-sm mb-6">
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    ...
                    className="mt-1 w-4 h-4 rounded border-zinc-300 text-[#4C1D95] focus:ring-[#4C1D95]/20"
                  />

...
import { useClientContracts } from '../../../client/services/clientHooks';
...
const WorkHistory = () => {
  ...
  const { data: response, isLoading: loading } = useClientContracts({ limit: 100 });

  const history = useMemo(() => {
    const raw = extractList(response);
    const completed = raw.filter((contract) => {
      const status = String(contract.status || '').toUpperCase();
      return ['COMPLETED', 'CLOSED', 'CANCELLED'].includes(status);
    });
    return completed.map((contract) => ({
      id: contract.id,
      jobId: contract.jobId,
      title: contract.jobTitle || 'Archived Contract',
      status: String(contract.status || '').toLowerCase(),
      date: contract.endDate || contract.updatedAt,
      provider: contract.providerName || 'Unknown Provider',
      amount: Number(contract.totalAmount || contract.amount || 0),

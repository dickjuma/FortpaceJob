      ...
      <div className="bg-surface min-h-[calc(100vh-80px)] flex flex-col items-center py-10">
        <div className="w-full max-w-3xl bg-white border border-zinc-200 rounded-3xl shadow-xl overflow-hidden flex flex-col h-[700px]">
          ...
          <div className="bg-surface-dark text-white p-6 flex items-center gap-4">
            <div className="w-12 h-12 bg-[#2bb75c] rounded-full flex items-center justify-center shadow-lg border-2 border-zinc-800">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-surface">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-4 max-w-[85%] ${msg.type === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm ${msg.type === 'user' ? 'bg-zinc-200 text-zinc-600' : 'bg-[#2bb75c] text-white'}`}>
                  {msg.type === 'user' ? <User className="w-5 h-5" /> : <Bot className="w-5 h-5" />}
                </div>
                <div>
                  <div className={`p-4 rounded-2xl shadow-sm ${msg.type === 'user' ? 'bg-[#2bb75c] text-white rounded-tr-sm' : 'bg-white border border-zinc-200 text-zinc-800 rounded-tl-sm'}`}>
                    {msg.text}
                  </div>
                  
                  {msg.action === 'show_profiles' && (
                    <div className="mt-4 space-y-3">
                      <div className="bg-white border border-[#2bb75c]/20 rounded-xl p-4 shadow-sm flex items-center justify-between hover:border-[#2bb75c]/20 cursor-pointer">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-sm text-zinc-900 flex items-center gap-1">Sarah W. <ShieldCheck className="w-3 h-3 text-[#2bb75c]" /></div>
            ...
            {isTyping && (
              <div className="flex gap-4 max-w-[85%]">
                <div className="w-10 h-10 rounded-full bg-[#2bb75c] text-white flex items-center justify-center shrink-0 shadow-sm">
          ...
          <div className="p-4 bg-white border-t border-zinc-200">
            <form onSubmit={handleSend} className="relative">
              <input 
                ...
                className="w-full bg-surface border border-zinc-200 rounded-2xl pl-6 pr-14 py-4 focus:outline-none focus:border-[#2bb75c]/20 focus:ring-1 focus:ring-[#2bb75c] shadow-inner"
              />
              <button 
                type="submit"
                className="absolute right-3 top-1/2 -tranzinc-y-1/2 w-10 h-10 bg-[#2bb75c] hover:bg-[#1d8d38] text-white rounded-xl flex items-center justify-center transition-colors shadow-md"
              >

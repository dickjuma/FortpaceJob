      ...
      <div className="bg-surface min-h-[calc(100vh-80px)] flex flex-col items-center py-10">
        <div className="w-full max-w-3xl bg-white border border-zinc-200 rounded-3xl shadow-xl overflow-hidden flex flex-col h-[700px]">
          ...
          <div className="bg-surface-dark text-white p-6 flex items-center gap-4">
            <div className="w-12 h-12 bg-brand-600 rounded-full flex items-center justify-center shadow-lg border-2 border-zinc-800">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-surface">
            {messages.map((msg, i) => (
              <div key={i} className={`flex gap-4 max-w-[85%] ${msg.type === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm ${msg.type === 'user' ? 'bg-zinc-200 text-zinc-600' : 'bg-brand-600 text-white'}`}>
                  {msg.type === 'user' ? <User className="w-5 h-5" /> : <Bot className="w-5 h-5" />}
                </div>
                <div>
                  <div className={`p-4 rounded-2xl shadow-sm ${msg.type === 'user' ? 'bg-brand-600 text-white rounded-tr-sm' : 'bg-white border border-zinc-200 text-zinc-800 rounded-tl-sm'}`}>
                    {msg.text}
                  </div>
                  
                  {msg.action === 'show_profiles' && (
                    <div className="mt-4 space-y-3">
                      <div className="bg-white border border-brand-200 rounded-xl p-4 shadow-sm flex items-center justify-between hover:border-brand-400 cursor-pointer">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <div className="font-bold text-sm text-zinc-900 flex items-center gap-1">Sarah W. <ShieldCheck className="w-3 h-3 text-brand-500" /></div>
            ...
            {isTyping && (
              <div className="flex gap-4 max-w-[85%]">
                <div className="w-10 h-10 rounded-full bg-brand-600 text-white flex items-center justify-center shrink-0 shadow-sm">
          ...
          <div className="p-4 bg-white border-t border-zinc-200">
            <form onSubmit={handleSend} className="relative">
              <input 
                ...
                className="w-full bg-surface border border-zinc-200 rounded-2xl pl-6 pr-14 py-4 focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 shadow-inner"
              />
              <button 
                type="submit"
                className="absolute right-3 top-1/2 -tranzinc-y-1/2 w-10 h-10 bg-brand-600 hover:bg-brand-700 text-white rounded-xl flex items-center justify-center transition-colors shadow-md"
              >

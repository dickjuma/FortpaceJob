          ...
          <div className="flex-1">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      <h1 className="text-2xl md:text-3xl font-bold text-zinc-900 flex items-center gap-2 mb-1">
                        {talent.name}
                        {talent.verified ? <ShieldCheck className="w-6 h-6 text-[#4C1D95]" title="Identity Verified" /> : null}
                      </h1>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <button className="w-10 h-10 rounded-full border border-zinc-200 flex items-center justify-center text-zinc-500 hover:text-[#4C1D95] hover:bg-[#4C1D95]/5 transition-colors" type="button">
                ...
                {tabs.map((tab) => (
                  <button
                    className={`px-6 py-4 font-medium text-sm whitespace-nowrap border-b-2 transition-colors ${
                      activeTab === tab ? 'border-[#4C1D95]/20 text-[#4C1D95]' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:bg-surface'
                    }`}
                    key={tab}
                ...
                {activeTab === 'services' ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {talent.services.map((service) => (
                      <div className="border border-zinc-200 rounded-xl p-5 hover:border-[#4C1D95]/50 transition-colors group" key={service.id}>
                        <div className="flex justify-between items-start mb-3 gap-3">
                          <h4 className="font-bold text-zinc-900 group-hover:text-[#4C1D95] transition-colors">{service.title}</h4>
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-5">
                ...
                <Link className="text-[#4C1D95] font-bold hover:text-[#4C1D95]" to="/search-results">
                  Search more talent
                </Link>
              </div>
          ...
          <div className="w-full lg:w-80 flex-shrink-0 lg:mt-24">
            <div className="sticky top-24 space-y-6">
              <div className="bg-white rounded-2xl p-6 shadow-xl border border-zinc-200">
                ...
                <button
                  className="w-full block text-center bg-[#4C1D95] hover:bg-[#22C55E] text-white font-bold py-3 px-4 rounded-xl transition-colors mb-3"
                  onClick={() =>

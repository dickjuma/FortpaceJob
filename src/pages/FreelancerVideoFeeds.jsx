  ...
  if (isLoading) {
    return (
      <div className="bg-black min-h-screen flex items-center justify-center">
        <Loader2 className="w-10 h-10 animate-spin text-[#14a800]" />
      </div>
    );
  }
            ...
            feed.map((video) => (
              <div key={video.id} className="w-full h-full snap-start relative bg-surface-dark border border-zinc-800 rounded-3xl overflow-hidden mb-4">
                ...
                <div className="absolute bottom-32 right-4 flex flex-col gap-6 z-10">
                  ...
                  <button className="group flex flex-col items-center gap-1">
                    <div className="w-12 h-12 bg-black/40 backdrop-blur rounded-full flex items-center justify-center text-white group-hover:text-[#14a800] transition-colors border border-white/10">
                ...
                <div className="absolute bottom-0 left-0 right-16 p-6 z-10">
                  <Link to={video.profilePath || '/profile'} className="flex items-center gap-2 mb-2">
                    ...
                    {video.verified && <ShieldCheck className="w-5 h-5 text-[#14a800] drop-shadow-md" />}
                  </Link>
                  ...
                  <div className="flex items-center gap-3">
                    <button className="flex-1 py-3 bg-[#14a800] hover:bg-[#118a00] text-white font-bold rounded-xl transition-colors shadow-lg shadow-blue-900/50 flex items-center justify-center gap-2">

          ...
          <div className="flex justify-between items-center mb-8 shrink-0">
            <div>
              <h1 className="text-2xl font-black text-zinc-900 flex items-center gap-2">
                <Columns className="w-6 h-6 text-brand-600" /> Hiring Pipeline
              </h1>
              <p className="text-zinc-600 text-sm mt-1">Drag and drop candidates across stages.</p>
            </div>
            <button className="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-bold rounded-lg transition-colors shadow-sm flex items-center gap-1">
              ...
              <div key={i} className="w-[350px] shrink-0 snap-start bg-zinc-100/50 border border-zinc-200 rounded-2xl flex flex-col max-h-full">
                ...
                <div className="p-3 overflow-y-auto flex-1 space-y-3 min-h-[200px]">
                  {column.cards.map((card, j) => (
                    <div key={j} className="bg-white border border-zinc-200 p-4 rounded-xl shadow-sm hover:border-brand-400 hover:shadow-md transition-all cursor-grab active:cursor-grabbing">

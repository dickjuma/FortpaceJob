          ...
          <div className="bg-white rounded-2xl border border-zinc-200 shadow-sm overflow-hidden">
            <div className="p-6 bg-surface-dark text-white flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[#2bb75c] text-sm">Responds in about {talent.responseTime}</p>
              </div>
            ...
            <div className="p-6 md:p-8 space-y-8">
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {openings.map((job) => (
                    <div
                      className={`p-4 border rounded-xl cursor-pointer transition-colors flex gap-3 ${selectedJob === job.id ? 'border-[#2bb75c]/20 bg-[#2bb75c]/5/50' : 'border-zinc-200 hover:border-[#2bb75c]/50'}`}
                      key={job.id}
                      onClick={() => setSelectedJob(job.id)}
                    >
                      <Briefcase className={`w-5 h-5 flex-shrink-0 mt-0.5 ${selectedJob === job.id ? 'text-[#2bb75c]' : 'text-zinc-400'}`} />
                      <div>
                  ...
                  <div
                    className={`p-4 border rounded-xl cursor-pointer transition-colors flex gap-3 ${selectedJob === 'new' ? 'border-[#2bb75c]/20 bg-[#2bb75c]/5/50' : 'border-zinc-200 hover:border-[#2bb75c]/50'}`}
                    onClick={() => setSelectedJob('new')}
                  >
                    <FileText className={`w-5 h-5 flex-shrink-0 mt-0.5 ${selectedJob === 'new' ? 'text-[#2bb75c]' : 'text-zinc-400'}`} />
                    <div>
              ...
              {selectedJob === 'new' ? (
                <div className="bg-surface border border-zinc-200 rounded-xl p-5">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <button className="text-xs font-medium text-[#2bb75c] flex items-center gap-1 hover:text-[#2bb75c]" type="button">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea
                    className="w-full bg-white border border-zinc-300 rounded-xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:border-[#2bb75c]/20 focus:ring-1 focus:ring-[#2bb75c]"
              ...
              <div className="pt-6 border-t border-zinc-100 flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                <div className="flex gap-3 w-full sm:w-auto">
                  ...
                  <button className="flex-1 sm:flex-none text-center px-8 py-2.5 bg-[#2bb75c] hover:bg-[#1d8d38] text-white font-bold rounded-xl shadow-md transition-colors" type="button">
                    Send invitation
                  </button>
                </div>
              </div>

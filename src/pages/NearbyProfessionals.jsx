      ...
      <div className="container mx-auto px-4 md:px-8 -mt-8 relative z-20 pb-20">
        <div className="bg-white rounded-2xl shadow-xl border border-zinc-200 p-4 mb-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex gap-2">
            <button type="button" onClick={() => setViewMode('map')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 ${viewMode === 'map' ? 'bg-[#2bb75c] text-white' : 'bg-zinc-100 text-zinc-600'}`}>
              <Map className="w-4 h-4" /> Map
            </button>
            <button type="button" onClick={() => setViewMode('list')} className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 ${viewMode === 'list' ? 'bg-[#2bb75c] text-white' : 'bg-zinc-100 text-zinc-600'}`}>

                ...
                {folders.map((folder) => (
                  <button
                    className={`w-full text-left rounded-2xl border px-4 py-4 transition-colors ${
                      activeFolder === folder.id ? 'border-brand-300 bg-brand-50/60' : 'border-zinc-200 bg-surface hover:bg-white'
                    }`}
                    key={folder.id}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="text-xs font-semibold text-brand-600">{folder.count}</div>
                    </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
              ...
              <div className="flex items-center gap-3">
                <div className="flex bg-white border border-zinc-200 rounded-xl p-1">
                  <button className={`p-2 rounded-lg ${viewMode === 'grid' ? 'bg-zinc-100 text-brand-600' : 'text-zinc-500'}`} onClick={() => setViewMode('grid')} type="button">
                    <LayoutGrid className="w-4 h-4" />
                  </button>
                  <button className={`p-2 rounded-lg ${viewMode === 'list' ? 'bg-zinc-100 text-brand-600' : 'text-zinc-500'}`} onClick={() => setViewMode('list')} type="button">
            ...
            {viewMode === 'grid' ? (
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {savedTalent.map((talent) => (
                  <div className="bg-white border border-zinc-200 rounded-3xl p-5 hover:border-brand-300 hover:shadow-lg transition-all" key={talent.id}>
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      />
                      <div>
                        <Link className="font-bold text-zinc-900 text-lg hover:text-brand-600 transition-colors" to={`/talent/${talent.id}`}>
                          {talent.name}
                        </Link>
                ...
                {savedTalent.map((talent) => (
                  <div className="bg-white border border-zinc-200 rounded-2xl p-5 flex flex-col md:flex-row gap-5 items-start md:items-center" key={talent.id}>
                    <div className="flex-1">
                      <Link className="text-xl font-bold text-zinc-900 hover:text-brand-600 transition-colors" to={`/talent/${talent.id}`}>
                        {talent.name}
                      </Link>

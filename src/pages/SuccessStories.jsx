    ...
    <>
      <div className="bg-surface min-h-screen py-16">
        <div className="container mx-auto px-4 md:px-8 max-w-6xl">
            ...
            {SUCCESS_STORIES.map((story, index) => (
              <div key={story.id} className={`bg-white rounded-3xl border border-zinc-200 overflow-hidden shadow-sm flex flex-col ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                ...
                <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                  ...
                  <h2 className="text-3xl font-black text-zinc-900 mb-6 leading-tight hover:text-[#4C1D95] transition-colors cursor-pointer">
                    {story.title}
                  </h2>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Link to={`/case-study/${story.id}`} className="flex items-center gap-1 text-[#4C1D95] font-bold hover:text-[#4C1D95] transition-colors">
          ...
          {/* CTA */}
          <div className="bg-[#4C1D95] rounded-3xl p-12 text-center text-white shadow-2xl relative overflow-hidden">
            <div className="absolute top-0 right-0 -mr-20 -mt-20 w-64 h-64 bg-[#4C1D95] rounded-full blur-3xl opacity-50"></div>
            <h2 className="text-3xl md:text-4xl font-black mb-6 relative z-10">Ready to write your success story?</h2>
            <p className="text-[#4C1D95] text-lg mb-8 max-w-2xl mx-auto relative z-10">
              Join thousands of forward-thinking companies building their dream teams on Fortspace.
            </p>
            <button className="px-8 py-4 bg-white text-[#4C1D95] font-bold rounded-xl shadow-lg hover:bg-surface transition-colors relative z-10">
              Start Hiring Today
            </button>
          </div>

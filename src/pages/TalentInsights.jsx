      ...
      <div className="bg-surface min-h-screen py-16">
        <div className="container mx-auto px-4 md:px-8 max-w-6xl">
          ...
          <div className="flex items-center gap-4 mb-10 border-b border-zinc-200 pb-6">
            <div className="w-14 h-14 bg-[#4C1D95]/10 text-[#4C1D95] rounded-2xl flex items-center justify-center shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
            {[
              ...
              <div key={i} className="bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm hover:border-[#4C1D95]/50 transition-colors">
                <div className="flex justify-between items-start mb-4">
                  <div className="w-10 h-10 bg-surface rounded-lg flex items-center justify-center">
                    <metric.icon className="w-5 h-5 text-zinc-500" />
                  </div>
                  <span className={`text-xs font-bold px-2 py-1 rounded-full ${metric.trend.startsWith('+') ? 'bg-emerald-100 text-emerald-700' : 'bg-[#4C1D95]/10 text-[#4C1D95]'}`}>
                    {metric.trend}
                  </span>
                </div>

...
import { Briefcase, CheckCircle2, Eye, MoreVertical, Users } from 'lucide-react';
import { Link } from 'react-router-dom';
import { getClientPostedJobs } from './findWorkData';
...
export default function ClientPostedWork() {
  ...
  const postings = getClientPostedJobs();
  const visibleJobs = postings.filter((posting) => posting.status === activeTab);

  const totalApplications = postings.reduce((sum, posting) => sum + posting.job.applicants, 0);
  const averageBudget = postings.length
    ? Math.round(postings.reduce((sum, posting) => sum + posting.job.budgetValue, 0) / postings.length)
    : 0;
      ...
      <div className="bg-surface min-h-screen py-10">
        <div className="container mx-auto px-4 md:px-8 max-w-7xl">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <Link to="/post-job" className="px-6 py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-sm transition-colors">
              Post a New Job
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
            ...
            <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
              ...
              <div className="text-3xl font-black text-brand-600">{totalApplications}</div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
              <div className="text-sm font-bold text-zinc-500 mb-1">Avg Budget</div>
              <div className="text-3xl font-black text-success">${averageBudget.toLocaleString()}</div>
            </div>
            <div className="bg-white p-6 rounded-2xl border border-zinc-200 shadow-sm">
              <div className="text-sm font-bold text-zinc-500 mb-1">Hire Rate</div>
              <div className="text-3xl font-black text-brand-600">85%</div>
            </div>
          </div>
              ...
              {['open', 'in_progress', 'completed', 'closed'].map((tab) => (
                <button
                  ...
                  className={`px-8 py-4 font-bold text-sm whitespace-nowrap transition-colors border-b-2 ${
                    activeTab === tab ? 'border-brand-600 text-brand-600 bg-white' : 'border-transparent text-zinc-500 hover:text-zinc-900 hover:bg-surface'
                  }`}
                >
                  ...
                  {visibleJobs.map((posting) => (
                    <tr key={posting.job.id} className="hover:bg-surface transition-colors block md:table-row border-b md:border-b-0 border-zinc-100 last:border-0 p-4 md:p-0">
                      <td className="p-2 md:p-6 block md:table-cell">
                        <Link to={posting.job.detailPath} className="font-bold text-lg text-zinc-900 hover:text-brand-600 transition-colors mb-1 block">
                          {posting.job.title}
                        </Link>
                      ...
                      <td className="p-2 md:p-6 md:text-center block md:table-cell">
                        ...
                        <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-brand-50 text-brand-700 font-bold rounded-lg border border-brand-100">
                      ...
                      <td className="p-2 md:p-6 md:text-right block md:table-cell">
                        <div className="flex items-center md:justify-end gap-2 mt-2 md:mt-0">
                          {posting.status === 'open' ? (
                            <Link to={`/find-work/work/${posting.job.id}/applications`} className="px-4 py-2 bg-brand-600 hover:bg-brand-700 text-white text-sm font-bold rounded-lg transition-colors shadow-sm">
                              Review
                            </Link>
                          ) : null}

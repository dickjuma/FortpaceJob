...
function renderStatusBadge(status) {
  switch (status) {
    ...
    case 'interviewing':
      return <span className="px-3 py-1 bg-[#14a800]/10 text-[#14a800] border border-[#14a800]/20 rounded-lg text-xs font-bold uppercase tracking-wider">Interviewing</span>;
    case 'accepted':
      return (
            ...
            <div className="flex border-b border-zinc-200 bg-surface/50">
              <button
                ...
                className={`flex-1 sm:flex-none px-8 py-4 font-bold text-sm transition-colors border-b-2 ${
                  activeTab === 'active' ? 'border-[#14a800]/20 text-[#14a800] bg-white' : 'border-transparent text-zinc-500 hover:text-zinc-900 hover:bg-surface'
                }`}
              >
            ...
            <div className="divide-y divide-zinc-100">
              {loading ? (
                ...
                <div className="py-20 text-center text-zinc-500">
                  ...
                  <Link to="/find-work" className="inline-block mt-4 text-[#14a800] font-bold hover:underline">
                    Browse open jobs
                  </Link>
                </div>
              ) : (
                  ...
                  <div key={application.id} className="p-6 hover:bg-surface transition-colors group">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                      <div className="flex-1">
                        ...
                        <Link to={application.job.detailPath} className="text-xl font-bold text-zinc-900 hover:text-[#14a800] transition-colors mb-1 block">
                      ...
                      <div className="flex flex-wrap md:flex-nowrap gap-2 w-full md:w-auto mt-4 md:mt-0">
                        {application.status === 'interviewing' ? (
                          <Link to="/messages" className="flex-1 md:flex-none px-6 py-2.5 bg-[#14a800] hover:bg-[#118a00] text-white font-bold rounded-xl shadow-sm transition-colors flex items-center justify-center gap-2">

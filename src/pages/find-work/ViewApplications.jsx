    ...
    <>
      <div className="bg-surface min-h-screen py-10">
        <div className="container mx-auto px-4 md:px-8 max-w-6xl">
          ...
          <Link to="/find-work/my-posted-work" className="inline-flex items-center gap-2 text-sm font-bold text-zinc-500 hover:text-[#14a800] transition-colors mb-6">
          ...
          <div className="bg-white border border-zinc-200 rounded-3xl p-6 md:p-8 shadow-sm mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex gap-4 text-center">
              <div className="bg-surface px-6 py-3 rounded-xl border border-zinc-100">
                <div className="text-2xl font-black text-[#14a800]">{summary.total}</div>
            ...
            <div className="lg:w-64 shrink-0 space-y-6">
              <div className="bg-white rounded-2xl border border-zinc-200 p-5 shadow-sm">
                ...
                <div className="space-y-2">
                  <button onClick={() => setFilter('all')} type="button" className={`w-full text-left px-3 py-2 rounded-lg font-bold text-sm transition-colors ${filter === 'all' ? 'bg-[#14a800]/5 text-[#14a800]' : 'text-zinc-600 hover:bg-surface'}`}>
                    All Applications
                  </button>
                  <button onClick={() => setFilter('new')} type="button" className={`w-full text-left px-3 py-2 rounded-lg font-bold text-sm transition-colors ${filter === 'new' ? 'bg-[#14a800]/5 text-[#14a800]' : 'text-zinc-600 hover:bg-surface'}`}>
                    New / Unread
                  </button>
            ...
            <div className="flex-1 space-y-6">
              {loading ? (
                <div className="bg-white border border-zinc-200 rounded-3xl py-20 text-center shadow-sm">
                  <Loader2 className="w-10 h-10 text-[#14a800] animate-spin mx-auto mb-4" />
                    ...
                    <div className="flex flex-col md:flex-row gap-6">
                      ...
                      <div className="md:w-1/3 flex flex-col items-center text-center border-b md:border-b-0 md:border-r border-zinc-100 pb-6 md:pb-0 md:pr-6 shrink-0">
                        {app.avatar ? (
                          ...
                          <div className="w-20 h-20 rounded-full mb-3 bg-[#14a800]/10 text-[#14a800] flex items-center justify-center text-2xl font-black">
                            {app.provider.charAt(0)}
                          </div>
                        )}
                        <h3 className="font-bold text-zinc-900 text-lg mb-1">{app.provider}</h3>
                        <div className="text-xs font-bold text-[#14a800] bg-[#14a800]/5 px-2 py-0.5 rounded uppercase tracking-wider mb-2">{app.badge}</div>
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex flex-wrap gap-3 mt-4">
                          <Link to="/messages" className="flex-1 px-4 py-2 bg-[#14a800] hover:bg-[#118a00] text-white text-sm font-bold rounded-lg shadow-sm transition-colors text-center">
                            Message
                          </Link>
                          {app.status !== 'shortlisted' ? (
                            <button

...
import { getFindWorkJobById, getRelatedFindWorkJobs } from './findWorkData';
      ...
      <div className="bg-surface min-h-screen py-10">
        <div className="container mx-auto px-4 md:px-8 max-w-7xl">
          <div className="flex items-center gap-2 text-sm font-medium text-zinc-500 mb-6 flex-wrap">
            <Link to="/find-work" className="hover:text-brand-600">Find Work</Link>
            <ChevronRight className="w-4 h-4" />
            <Link to={job.workMode === 'local' ? '/find-work/local' : '/find-work/online'} className="hover:text-brand-600">
              {job.workMode === 'local' ? 'Local' : 'Online'}
            </Link>
            <ChevronRight className="w-4 h-4" />
            <Link to={job.categoryPath} className="hover:text-brand-600">{job.category.name}</Link>
          </div>
          ...
          <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="bg-white border border-zinc-200 rounded-3xl p-8 shadow-sm">
                <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6 border-b border-zinc-100 pb-6">
                  ...
                  <div className="flex gap-2 w-full md:w-auto">
                    <Link to="/find-work/saved" className="p-3 bg-white border border-zinc-200 text-zinc-600 hover:text-brand-600 hover:border-brand-300 rounded-xl transition-colors shadow-sm">
                      <Bookmark className="w-5 h-5" />
                    </Link>
                    <button type="button" className="p-3 bg-white border border-zinc-200 text-zinc-600 hover:text-brand-600 hover:border-brand-300 rounded-xl transition-colors shadow-sm">
                ...
                <div className="bg-brand-50 border border-brand-100 rounded-2xl p-6 flex items-start gap-4">
                  <FileText className="w-6 h-6 text-brand-600 shrink-0 mt-1" />
                  <div className="flex-1">
                    <h4 className="font-bold text-zinc-900 mb-1">Attachments (1)</h4>
                    <button type="button" className="text-brand-600 text-sm font-bold hover:underline text-left">
                      {job.attachment.name}
                    </button>
              ...
              {relatedJobs.length > 0 ? (
                <div className="bg-white border border-zinc-200 rounded-3xl p-8 shadow-sm">
                  <div className="flex justify-between items-center gap-4 mb-6">
                    ...
                    <Link to={`/find-work/search?type=${job.workMode}&q=${encodeURIComponent(job.category.name)}`} className="text-brand-600 font-bold text-sm hover:underline">
                      See all
                    </Link>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {relatedJobs.map((relatedJob) => (
                      <Link key={relatedJob.id} to={relatedJob.detailPath} className="border border-zinc-200 rounded-2xl p-5 hover:border-brand-300 hover:shadow-sm transition-all group">
                        <div className="text-xs font-bold uppercase tracking-wider text-brand-600 mb-2">{relatedJob.specialization}</div>
                        <h4 className="font-bold text-zinc-900 group-hover:text-brand-600 transition-colors mb-2">{relatedJob.title}</h4>
            ...
            <div className="lg:w-1/3 space-y-6">
              <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-xl sticky top-24">
                ...
                <Link to={job.proposalPath} className="w-full py-4 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl shadow-md transition-colors text-lg mb-4 block text-center">
                  Submit a Proposal
                </Link>
              ...
              <div className="bg-white border border-zinc-200 rounded-3xl p-6 shadow-sm">
                ...
                <Link to="/client-profile" className="text-brand-600 text-sm font-bold hover:underline block text-center pt-4 border-t border-zinc-100">

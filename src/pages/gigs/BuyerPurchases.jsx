            ...
            <div className="flex border-b border-zinc-200 bg-surface/50">
              {['active', 'completed', 'cancelled'].map((tab) => (
                <button
                  ...
                  className={`flex-1 sm:flex-none px-8 py-4 font-bold text-sm transition-colors border-b-2 capitalize ${activeTab === tab ? 'border-[#4C1D95]/20 text-[#4C1D95] bg-white' : 'border-transparent text-zinc-500 hover:text-zinc-900 hover:bg-surface'}`}
                >
                  {tab} Orders
                </button>
              ))}
                    ...
                    <div key={order.id} className="border border-zinc-200 rounded-2xl p-6 hover:shadow-md transition-shadow">
                      <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-4">
                        <div>
                          ...
                          <Link to={`/gigs/gig/${order.gigId || ''}`} className="text-lg font-bold text-zinc-900 hover:text-[#4C1D95] transition-colors block mb-1">
                            {order.gig}
                          </Link>
                      ...
                      <div className="bg-surface border border-zinc-100 rounded-xl p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
                        <div className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full ${order.status === 'delivered' ? 'bg-[#4C1D95]' : order.status === 'completed' ? 'bg-success' : 'bg-amber-500'}`}></span>
                        ...
                        <div className="flex w-full sm:w-auto gap-2">
                          ...
                          {order.status === 'delivered' && (
                            <>
                              <button type="button" className="flex-1 sm:flex-none px-4 py-2.5 bg-[#4C1D95]/10 text-[#4C1D95] font-bold rounded-lg flex items-center justify-center gap-2">

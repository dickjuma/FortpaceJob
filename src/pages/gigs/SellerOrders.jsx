...
const SellerOrders = () => {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      case 'in_progress': return 'bg-blue-100 text-blue-700 dark:bg-[#14a800]/20 dark:text-blue-400 border-blue-200';
      case 'delivered': return 'bg-[#14a800]/10 text-[#14a800] border-[#14a800]/30';
          ...
          {TABS.map((tab) => (
            <button
              ...
              className={`relative px-6 py-4 font-bold text-sm whitespace-nowrap transition-colors capitalize ${
                activeTab === tab
                  ? 'text-[#14a800] dark:text-[#14a800] bg-white dark:bg-zinc-900'
            ...
            >
              ...
              {activeTab === tab && (
                <motion.div layoutId="orderTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-[#14a800]" />
              )}
            </button>
          ))}
        </div>
              ...
              >
                {displayOrders.length === 0 ? (
                  ...
                  displayOrders.map((order) => (
                    <div key={order.id} className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 hover:border-[#14a800]/50 transition-all flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 group">
                      <div className="flex-1 w-full">
                        ...
                        <div className="flex items-center gap-2 text-sm font-medium text-zinc-500 dark:text-zinc-400">
                          ...
                          Tier: <span className="text-[#14a800] font-bold">{order.tier}</span>
                        </div>
                      </div>
                      <div className="w-full lg:w-auto flex flex-col sm:flex-row items-center gap-4 pt-4 lg:pt-0 border-t lg:border-t-0 border-zinc-100 dark:border-zinc-800">
                        ...
                        <div className="flex w-full sm:w-auto gap-2">
                          ...
                          {order.status === 'in_progress' && (
                            <button type="button" className="flex-1 sm:flex-none px-6 py-3 bg-[#14a800] hover:bg-[#118a00] text-white font-semibold rounded-xl transition-all text-center whitespace-nowrap">
                              Deliver Work
                            </button>
                          )}

...
const performanceMetrics = [
  ...
  { label: 'Avg Contract Size', value: '$4,250', trend: '+12.5%', color: 'text-[#4C1D95]' },
        ...
        <div className="flex gap-3 items-center">
          <div className="relative">
            ...
            <select 
              ...
              className="pl-9 pr-8 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm font-semibold focus:ring-2 focus:ring-[#4C1D95] outline-none appearance-none"
            >
          ...
          <button className="px-5 py-2 bg-[#4C1D95] hover:bg-[#22C55E] text-white rounded-lg text-sm font-bold shadow-sm transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-700 shadow-sm flex flex-col">
           <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-lg text-zinc-900 dark:text-white flex items-center gap-2">
              <Target className="w-5 h-5 text-[#4C1D95]" /> Weekly Productivity Heatmap
            </h2>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-2">
               ...
               <div key={day} className="flex items-center gap-3">
                 ...
                 <div className="flex-1 flex gap-1.5">
                   {Array.from({ length: 24 }).map((_, i) => {
                     ...
                     if (intensity > 0.8) bgClass = 'bg-[#4C1D95]'; // level 4
                     else if (intensity > 0.6) bgClass = 'bg-[#4C1D95]'; // level 3
                     else if (intensity > 0.4) bgClass = 'bg-[#4C1D95]'; // level 2
                     else if (intensity > 0.2) bgClass = 'bg-[#4C1D95]'; // level 1

                     return (
                       <div key={i} className={`h-6 flex-1 rounded-sm ${bgClass} transition-colors hover:ring-2 hover:ring-[#4C1D95] cursor-pointer`} title={`Hour: ${i}:00`} />
                     );
                   })}
                 </div>
               </div>
             ...
             <div className="flex items-center justify-end gap-2 mt-4 text-[10px] font-bold text-zinc-500">
               Less <div className="flex gap-1"><span className="w-3 h-3 rounded-sm bg-zinc-100 dark:bg-zinc-700/50" /><span className="w-3 h-3 rounded-sm bg-[#4C1D95]" /><span className="w-3 h-3 rounded-sm bg-[#4C1D95]" /><span className="w-3 h-3 rounded-sm bg-[#4C1D95]" /></div> More
             </div>
          </div>
        </div>

...
const performanceMetrics = [
  ...
  { label: 'Avg Contract Size', value: '$4,250', trend: '+12.5%', color: 'text-brand-500' },
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-1 flex items-center gap-3">
            <BarChart2 className="w-8 h-8 text-brand-600" /> Intelligence & Analytics
          </h1>
        ...
        <div className="flex gap-3 items-center">
          <div className="relative">
            ...
            <select 
              ...
              className="pl-9 pr-8 py-2 bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-lg text-sm font-semibold focus:ring-2 focus:ring-brand-500 outline-none appearance-none"
            >
          ...
          <button className="px-5 py-2 bg-brand-600 hover:bg-brand-700 text-white rounded-lg text-sm font-bold shadow-sm transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-700 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-2">
            <h2 className="font-bold text-lg text-zinc-900 dark:text-white flex items-center gap-2">
              <PieChartIcon className="w-5 h-5 text-brand-500" /> Revenue by Category
            </h2>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-800 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-700 shadow-sm flex flex-col">
           <div className="flex justify-between items-center mb-4">
            <h2 className="font-bold text-lg text-zinc-900 dark:text-white flex items-center gap-2">
              <Target className="w-5 h-5 text-brand-500" /> Weekly Productivity Heatmap
            </h2>
          </div>
          ...
          <div className="flex-1 flex flex-col gap-2">
               ...
               <div key={day} className="flex items-center gap-3">
                 ...
                 <div className="flex-1 flex gap-1.5">
                   {Array.from({ length: 24 }).map((_, i) => {
                     ...
                     if (intensity > 0.8) bgClass = 'bg-brand-600'; // level 4
                     else if (intensity > 0.6) bgClass = 'bg-brand-500'; // level 3
                     else if (intensity > 0.4) bgClass = 'bg-brand-400'; // level 2
                     else if (intensity > 0.2) bgClass = 'bg-brand-300'; // level 1

                     return (
                       <div key={i} className={`h-6 flex-1 rounded-sm ${bgClass} transition-colors hover:ring-2 hover:ring-indigo-300 cursor-pointer`} title={`Hour: ${i}:00`} />
                     );
                   })}
                 </div>
               </div>
             ...
             <div className="flex items-center justify-end gap-2 mt-4 text-[10px] font-bold text-zinc-500">
               Less <div className="flex gap-1"><span className="w-3 h-3 rounded-sm bg-zinc-100 dark:bg-zinc-700/50" /><span className="w-3 h-3 rounded-sm bg-brand-300" /><span className="w-3 h-3 rounded-sm bg-brand-500" /><span className="w-3 h-3 rounded-sm bg-brand-600" /></div> More
             </div>
          </div>
        </div>

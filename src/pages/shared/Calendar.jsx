...
// Mock Event Data
const mockEvents = [
  { id: 1, title: 'UX Review Sync', type: 'Meeting', time: '10:00 AM', duration: 1, day: 15, color: 'bg-brand-500' },
  { id: 2, title: 'Phase 1 Delivery', type: 'Milestone', time: '5:00 PM', duration: 1, day: 15, color: 'bg-green-500' },
  { id: 3, title: 'Interview: Frontend Dev', type: 'Interview', time: '2:30 PM', duration: 1, day: 16, color: 'bg-brand-500' },
  { id: 4, title: 'Project Proposal Due', type: 'Deadline', time: '11:59 PM', duration: 1, day: 18, color: 'bg-red-500' },
  { id: 5, title: 'Weekly Standup', type: 'Meeting', time: '9:00 AM', duration: 1, day: 19, color: 'bg-brand-500' },
];
    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12 h-[calc(100vh-80px)] flex flex-col">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shrink-0">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-2xl bg-brand-600 flex items-center justify-center shadow-lg shadow-blue-500/20 text-white shrink-0">
        ...
        <div className="flex items-center gap-4">
          ...
          <button className="bg-brand-600 hover:bg-brand-700 text-white px-4 py-2 rounded-xl font-bold shadow-lg shadow-blue-500/20 transition-all flex items-center gap-2">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        ...
        <div className="hidden lg:flex w-64 shrink-0 flex-col gap-6 overflow-y-auto custom-scrollbar pr-2">
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-5 shadow-sm">
            ...
            <div className="space-y-3">
              <label className="flex items-center gap-3 cursor-pointer group">
                <input type="checkbox" defaultChecked className="w-4 h-4 text-brand-600 rounded border-zinc-300 focus:ring-brand-500" />
          ...
          <div className="bg-gradient-to-br from-indigo-900 to-blue-900 rounded-2xl p-5 text-white shadow-xl relative overflow-hidden">
             ...
             <h3 className="font-bold text-sm mb-2 flex items-center gap-2 relative z-10">
               <Video className="w-4 h-4 text-brand-300" /> Next Meeting
             </h3>
             <div className="relative z-10">
               <p className="font-extrabold text-lg leading-tight mb-1">UX Review Sync</p>
               <p className="text-brand-200 text-xs font-medium mb-4">Starts in 15 mins</p>
          ...
          <div className="flex-1 grid grid-cols-7 grid-rows-5 overflow-hidden bg-zinc-100 dark:bg-zinc-700/30 gap-[1px]">
            {daysInMonth.map((dayNum, index) => {
              ...
              return (
                <div 
                  key={index} 
                  className={`bg-white dark:bg-zinc-800 p-2 flex flex-col transition-colors group cursor-crosshair hover:bg-brand-50/30 dark:hover:bg-zinc-700/50 ${!isCurrentMonth ? 'opacity-40 bg-surface dark:bg-surface-dark' : ''}`}
                >
                  <div className="flex justify-between items-start mb-1">
                    <span className={`w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold ${isToday ? 'bg-brand-600 text-white shadow-md' : 'text-zinc-700 dark:text-zinc-300 group-hover:text-brand-600'}`}>

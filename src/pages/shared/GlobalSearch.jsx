...
const GlobalSearch = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="bg-gradient-to-r from-blue-900 via-[#4C1D95] to-zinc-900 rounded-[2rem] p-8 md:p-12 text-white shadow-xl relative overflow-hidden">
        <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10 mix-blend-overlay" />
        <div className="absolute top-0 right-0 w-96 h-96 bg-[#4C1D95]/20 rounded-full blur-[100px] pointer-events-none" />
        
        <div className="relative z-10 max-w-3xl mx-auto text-center mb-8">
          <h1 className="text-3xl md:text-5xl font-extrabold mb-4 tracking-tight">
            Find the world's best talent.
          </h1>
          <p className="text-[#4C1D95]/80 text-lg">AI-powered matchmaking connecting you to elite professionals.</p>
        </div>

        <div className="relative z-10 max-w-4xl mx-auto">
          {/* Main Search Bar */}
          <div className={`relative bg-white dark:bg-surface-dark rounded-2xl flex items-center p-2 shadow-2xl transition-all duration-300 ${isFocused ? 'ring-4 ring-#4C1D95]/30 shadow-blue-900/50' : ''}`}>
            ...
            <button 
              ...
              className={`hidden sm:flex items-center gap-2 px-4 py-2 mx-2 rounded-xl text-sm font-bold transition-colors ${filtersOpen ? 'bg-[#4C1D95]/10 dark:bg-[#4C1D95]/50 text-[#4C1D95] dark:text-[#4C1D95]' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700'}`}
            >
              <SlidersHorizontal className="w-4 h-4" /> Filters
            </button>
            <button className="bg-[#4C1D95] hover:bg-[#22C55E] text-white px-8 py-3 rounded-xl font-bold transition-colors flex items-center gap-2 shadow-lg shadow-#4C1D95]/20 shrink-0">
            ...
            {isFocused && !searchQuery && (
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {suggestions.map(s => (
                    <button key={s} onClick={() => setSearchQuery(s)} className="px-3 py-1.5 bg-surface dark:bg-surface-dark border border-zinc-200 dark:border-zinc-700 text-zinc-600 dark:text-zinc-300 text-sm font-medium rounded-lg hover:border-[#4C1D95]/20 hover:text-[#4C1D95] dark:hover:text-[#4C1D95] transition-colors">
                      {s}
                    </button>
                  ))}
                </div>
              ...
              <div className="p-5 space-y-6">
                  ...
                  <div className="space-y-2">
                    {['Any amount', '$10 - $30/hr', '$30 - $60/hr', '$60 & above'].map((rate, i) => (
                      <label key={rate} className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-4 h-4 rounded border flex items-center justify-center transition-colors ${i === 2 ? 'bg-[#4C1D95] border-[#4C1D95]/20' : 'border-zinc-300 dark:border-zinc-600 group-hover:border-[#4C1D95]/20'}`}>
                  ...
                  <div className="space-y-2">
                    {['Any Job Success', '80% & up', '90% & up'].map((score, i) => (
                      <label key={score} className="flex items-center gap-3 cursor-pointer group">
                        <div className={`w-4 h-4 rounded-full border flex items-center justify-center transition-colors ${i === 2 ? 'border-[#4C1D95]/20' : 'border-zinc-300 dark:border-zinc-600 group-hover:border-[#4C1D95]/20'}`}>
                          {i === 2 && <div className="w-2 h-2 rounded-full bg-[#4C1D95]" />}
                        </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Type a country..." className="w-full pl-9 pr-3 py-2 bg-surface dark:bg-surface-dark border border-zinc-200 dark:border-zinc-700 rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#4C1D95]" />
                  </div>
                </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="space-y-4">
            {mockResults.map((result) => (
              <div key={result.id} className="bg-white dark:bg-zinc-800 border border-zinc-200 dark:border-zinc-700 rounded-3xl p-6 shadow-sm hover:border-[#4C1D95]/50 dark:hover:border-[#4C1D95]/50 hover:shadow-md transition-all group">
                <div className="flex flex-col sm:flex-row gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h2 className="text-xl font-bold text-zinc-900 dark:text-white group-hover:text-[#4C1D95] transition-colors flex items-center gap-2">
                          {result.name}
                        </h2>
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-700/50">
                      <div className="flex gap-2">
                        {result.tags.map(tag => (
                          <span key={tag} className="px-2 py-1 bg-[#4C1D95]/5 dark:bg-[#4C1D95]/20 text-[#4C1D95] dark:text-[#4C1D95] text-[10px] font-bold uppercase tracking-wider rounded">
                            {tag}
                          </span>
                        ))}
                      </div>
                      <div className="flex gap-2 w-full sm:w-auto">
                        ...
                        <button className="flex-1 sm:flex-none px-6 py-2 bg-[#4C1D95] hover:bg-[#22C55E] text-white text-sm font-bold rounded-xl transition-colors">
                          Hire Now
                        </button>
                      </div>
                    </div>

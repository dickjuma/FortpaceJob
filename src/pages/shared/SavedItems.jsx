    ...
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-bold mb-1 flex items-center gap-3">
            <Bookmark className="w-8 h-8 text-[#14a800] fill-blue-600" /> Saved Items
          </h1>
      ...
      <div className="flex overflow-x-auto border-b border-zinc-200 dark:border-zinc-800 mb-6 custom-scrollbar">
        {[
          ...
          >
            ...
            <span className={`px-2 py-0.5 rounded-full text-xs ${activeTab === tab.id ? 'bg-[#14a800]/10 text-[#14a800] dark:bg-[#14a800]/30' : 'bg-zinc-100 text-zinc-500 dark:bg-zinc-800'}`}>
              {tab.count}
            </span>
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <AnimatePresence mode="popLayout">
            ...
            {activeTab === 'Jobs' && itemsList.jobs.map((job) => (
              <motion.div 
                ...
                key={job.id} 
                className="bg-white dark:bg-zinc-800 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-700 shadow-sm hover:border-[#14a800]/50 dark:hover:border-[#14a800]/50 transition-colors"
              >
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-bold text-lg text-zinc-900 dark:text-white hover:text-[#14a800] cursor-pointer transition-colors mb-1">{job.title}</h3>
                    <p className="text-sm font-semibold text-zinc-500">{job.company}</p>
                  </div>
                  <button onClick={() => removeJob(job.id)} className="p-2 text-[#14a800] hover:bg-[#14a800]/5 dark:hover:bg-[#14a800]/20 rounded-full transition-colors">
                ...
                <div className="pt-4 border-t border-zinc-100 dark:border-zinc-700/50">
                  <button className="w-full sm:w-auto px-6 py-2.5 bg-[#14a800] hover:bg-[#118a00] text-white text-sm font-bold rounded-xl transition-colors shadow-lg shadow-#14a800]/20">
                    Apply Now
                  </button>
                </div>
              </motion.div>
            ...
            {activeTab === 'Freelancers' && itemsList.freelancers.map((f) => (
               <motion.div 
                 layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
                 key={f.id} 
                 className="bg-white dark:bg-zinc-800 rounded-3xl p-6 border border-zinc-200 dark:border-zinc-700 shadow-sm hover:border-[#14a800]/50 dark:hover:border-[#14a800]/50 transition-colors flex flex-col sm:flex-row gap-6"
               >
                 ...
                 <div className="flex-1">
                   <div className="flex justify-between items-start mb-2">
                     <div>
                       <h3 className="font-bold text-lg text-zinc-900 dark:text-white hover:text-[#14a800] cursor-pointer transition-colors mb-1">{f.name}</h3>
                       <p className="text-sm font-semibold text-zinc-500">{f.title}</p>
                     </div>
                     <button onClick={() => removeFreelancer(f.id)} className="p-2 text-[#14a800] hover:bg-[#14a800]/5 dark:hover:bg-[#14a800]/20 rounded-full transition-colors shrink-0">
                       <Bookmark className="w-5 h-5 fill-blue-600" />
                     </button>
                   </div>
              ...
              <motion.div 
               ...
               >
                 <div className="flex justify-between items-center mb-3">
                   ...
                   <button className="text-[#14a800] hover:text-[#14a800] text-sm font-bold">Run Search</button>
                 </div>
                 <div className="flex flex-wrap items-center gap-2 mb-4">
                   <span className="text-xs font-semibold text-zinc-500">Filters Applied:</span>
                   {s.filters.map(f => <span key={f} className="px-2 py-0.5 bg-[#14a800]/5 text-[#14a800] dark:bg-[#14a800]/30 dark:text-[#14a800] text-[10px] font-bold rounded uppercase">{f}</span>)}
                 </div>
        ...
        <div className="space-y-6">
          <div className="bg-gradient-to-br from-[#14a800] to-zinc-900 rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">

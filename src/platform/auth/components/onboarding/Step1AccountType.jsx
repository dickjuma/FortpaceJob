      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full mb-12">
        ...
        <motion.div
          ...
          className={`relative cursor-pointer overflow-hidden rounded-3xl p-8 transition-all duration-300 border-2 ${
            accountType === 'client' 
              ? 'border-[#4C1D95] bg-[#4C1D95]/5/50 dark:bg-[#4C1D95]/10 shadow-xl shadow-[#4C1D95]/25' 
              : 'border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:border-[#4C1D95]/50 dark:hover:border-[#4C1D95] hover:shadow-lg'
          }`}
        >
          {accountType === 'client' && (
            <div className="absolute inset-0 bg-gradient-to-br from-[#4C1D95]/5 to-#4C1D95]/5 pointer-events-none" />
          )}
          <div className="relative z-10 flex flex-col h-full">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-colors ${
              accountType === 'client' ? 'bg-[#4C1D95] text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
            }`}>
            ...
            <p className="text-zinc-600 dark:text-zinc-400 mb-6 flex-grow">
              Hire freelancers, agencies, businesses, and onsite professionals.
            </p>
            <ul className="space-y-3 mt-auto">
              {['Post jobs', 'Hire talent', 'Manage contracts', 'Build teams'].map((feature, i) => (
                <li key={i} className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  <div className={`w-1.5 h-1.5 rounded-full mr-3 ${accountType === 'client' ? 'bg-[#4C1D95]' : 'bg-zinc-300 dark:bg-zinc-600'}`} />
                  {feature}
                </li>
              ))}
            </ul>
        ...
        >
          ...
          <div className="relative z-10 flex flex-col h-full">
            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 transition-colors ${
              accountType === 'freelancer' ? 'bg-[#4C1D95] text-white shadow-md' : 'bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300'
            }`}>
            ...
            <ul className="space-y-3 mt-auto">
              {['Find work', 'Create gigs', 'Get paid', 'Build portfolio'].map((feature, i) => (
                <li key={i} className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  <div className={`w-1.5 h-1.5 rounded-full mr-3 ${accountType === 'freelancer' ? 'bg-[#4C1D95]' : 'bg-zinc-300 dark:bg-zinc-600'}`} />
                  {feature}
                </li>
              ))}
            </ul>
      ...
      <div className="flex justify-end w-full sm:w-auto sm:ml-auto">
        <button
          ...
          className={`group flex items-center justify-center w-full sm:w-auto px-8 py-4 rounded-2xl font-bold text-lg transition-all duration-300 ${
            accountType 
              ? 'bg-gradient-to-r from-[#4C1D95] to-blue-600 text-white shadow-lg shadow-[#4C1D95]/25 hover:shadow-[#4C1D95]/25 hover:-tranzinc-y-1' 

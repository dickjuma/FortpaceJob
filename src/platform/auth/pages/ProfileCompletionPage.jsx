...
import { onboardingAPI, profileAPI, taxonomyAPI } from '../../common/services/api';
...
export default function ProfileCompletionPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    skillsText: listToText(user?.skills),
    languagesText: listToText(user?.languages),
    ...
    hiringType: user?.hiringType || 'PROJECT_BASED',
  });
  ...
  useEffect(() => {
    ...
    Promise.all([
      onboardingAPI.getStatus().catch(() => blankBackendStatus),
      loadOnboardingDraft()
    ])
      .then(([response, draft]) => {
        if (!mounted) return;
        ...
        if (draft) {
          setFormData((prev) => ({
            ...prev,
            ...draft,
            skillsText: draft.skills ? listToText(draft.skills) : prev.skillsText,
            languagesText: draft.languages ? listToText(draft.languages) : prev.languagesText,
          }));
        }
  ...
  const selectedCategoryNode = useMemo(() => {
    return categoryTree.find((c) => String(c.id) === String(formData.categoryId));
  }, [categoryTree, formData.categoryId]);

  const selectedSubcategoryNode = useMemo(() => {
    return selectedCategoryNode?.children?.find((sc) => String(sc.id) === String(formData.subcategoryId));
  }, [selectedCategoryNode, formData.subcategoryId]);

  const categoryOptions = useMemo(() => {
    return categoryTree.map((c) => ({ value: String(c.id), label: c.name }));
  }, [categoryTree]);

  const subcategoryOptions = useMemo(() => {
    if (!selectedCategoryNode?.children) return [];
    return selectedCategoryNode.children.map((sc) => ({ value: String(sc.id), label: sc.name }));
  }, [selectedCategoryNode]);

  const serviceOptions = useMemo(() => {
    if (!selectedSubcategoryNode?.children) return [];
    return selectedSubcategoryNode.children.map((s) => ({ value: String(s.id), label: s.name }));
                            ...
                            <div className="grid gap-5 md:grid-cols-2">
                              ...
                              <div className="md:col-span-2 grid gap-5 md:grid-cols-3">
                                ...
                                <SelectField
                                  ...
                                  onChange={(event) => {
                                    updateField('subcategoryId', event.target.value);
                                    updateField('serviceId', '');
                                  }}
...
function SelectField({ label, icon: Icon, helper, options = [], ...props }) {
  return (
    <label className="block">
      <span className="mb-2 flex items-center gap-2 text-sm font-bold text-zinc-900 dark:text-white">
        {Icon ? <Icon className="h-4 w-4 text-[#4C1D95]" /> : null}
      ...
      <select
        className={cn(
          'w-full appearance-none rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition-all focus:border-[#4C1D95]/20 focus:ring-2 focus:ring-[#4C1D95]/10 dark:border-zinc-800 dark:bg-zinc-950 dark:text-white disabled:opacity-50 disabled:cursor-not-allowed',
          props.className
        )}
        {...props}
      >

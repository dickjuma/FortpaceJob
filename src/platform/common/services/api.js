...
const apiClient = async (endpoint, options = {}) => {
  ...
  if (!response.ok) {
    const fallbackMessage = response.status === 429
      ...
      : "An error occurred";
    const error = new Error(getErrorMessage(rawData, fallbackMessage));
    error.code = rawData?.code;
    error.status = response.status;
    error.statusCode = rawData?.statusCode || response.status;
    if (rawData?.metadata) error.metadata = rawData.metadata;
    throw error;
  }

  if (rawData?.success === false) {
    const error = new Error(getErrorMessage(rawData));
    error.code = rawData?.code;
    error.status = response.status;
    error.statusCode = rawData?.statusCode || response.status;
    if (rawData?.metadata) error.metadata = rawData.metadata;
...
export const taxonomyAPI = {
  getCategoryTree: async () => apiClient('/category-taxonomy/category/tree'),
};
...
export const authAPI = {
  ...
  verifyAdmin2FA: async (challengeId, totpCode, deviceInfo) => {
    const data = await apiClient("/admin/auth/verify-totp", {
      method: "POST",
      body: JSON.stringify({ challengeId, totpCode, deviceInfo }),
    });
    if (data?.accessToken) {
      setTokens(data.accessToken, data.refreshToken);
  ...
  verifyAdminEmailOtp: async (challengeId, code) => {
    const data = await apiClient("/admin/auth/verify-email-otp", {
      method: "POST",
...
export const profileAPI = {
  getMyProfile: async () => {
    ...
    const endpoint = role === 'CLIENT' ? '/profilesystem/client/me' : '/profilesystem/me';
  ...
  updateMyProfile: async (input) => {
    ...
    if (role === "FREELANCER") {
      endpoint = `/profilesystem/freelancer/profile`;
    } else if (role === "CLIENT") {
      endpoint = `/profilesystem/profile/client`;
    } else if (role === "AGENCY") {
      endpoint = `/profilesystem/profile/agency`;
    }
    ...
    const updatedUser = { ...user, ...data, profile: data };
    setUser(updatedUser);
    return { ...updatedUser, missingFields: [] };
  },
...
export const onboardingAPI = {
  ...
  saveDraft: async (step, data = {}) => {
    return apiClient('/profilesystem/onboarding/draft', {
      method: 'POST',
      body: JSON.stringify({ step, data }),
    });
  },
  ...
  getDraft: async () => {
    return apiClient('/profilesystem/onboarding/draft', { method: 'GET' });
  },
};
...
export const orderAPI = {
  ...
  acceptOrder: async (orderId) => apiClient(`/orders/${orderId}/accept`, { method: "POST" }),

  rejectOrder: async (orderId, reason) => apiClient(`/orders/${orderId}/reject`, {
    method: "POST",
    body: JSON.stringify({ reason }),
  }),

  deliverOrder: async (orderId, data) => apiClient(`/orders/${orderId}/deliver`, {
    method: "POST",
    body: JSON.stringify(data),
  }),

  requestRevision: async (orderId, data) => apiClient(`/orders/${orderId}/revision`, {
    method: "POST",
    body: JSON.stringify(data),
  }),

  approveRevision: async (orderId, revisionId, data) => apiClient(`/orders/${orderId}/revisions/${revisionId}/approve`, {
    method: "POST",
    body: JSON.stringify(data),
  }),

  completeOrder: async (orderId) => apiClient(`/orders/${orderId}/complete`, { method: "PATCH" }),

  cancelOrder: async (orderId, reason) => apiClient(`/orders/${orderId}/cancel`, {
    method: "POST",
    body: JSON.stringify({ reason }),
  }),

  disputeOrder: async (orderId, data) => apiClient(`/orders/${orderId}/dispute`, {
    method: "POST",
    body: JSON.stringify(data),
  }),
  ...
  adminListOrders: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiClient(`/orders/admin/all?${queryString}`);
  },

  refundOrder: async (orderId, reason) => apiClient(`/orders/admin/${orderId}/refund`, {
    method: "POST",
    body: JSON.stringify({ reason }),
  }),
};
...
export const gigAPI = {
  getGigs: async (params = {}) => {
    ...
    if (token) {
      try {
        return await apiClient(`/gigs?${queryString}`);
      } catch {
        // Fall through to public browse when session is invalid or listing is restricted.
      }
    }
    return apiClient(`/gigs/public?${queryString}`);
  },

  getMyGigs: async () => {
    return apiClient(`/gigs/my`);
  },
  ...
  getGig: async (gigId) => {
    ...
    if (token) {
      try {
        return await apiClient(`/gigs/${gigId}`);
      } catch {
        // Fall through to public detail endpoint.
      }
    }
    return apiClient(`/gigs/public/${gigId}`);
  },
  ...
  pauseGig: async (gigId) => {
    return apiClient(`/gigs/${gigId}/pause`, { method: "POST" });
  },

  activateGig: async (gigId) => {
    return apiClient(`/gigs/${gigId}/activate`, { method: "POST" });
  },
  ...
  getFeaturedGigs: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiClient(`/gigs/featured?${queryString}`);
  },

  getRecommendedGigs: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiClient(`/gigs/recommended?${queryString}`);
  },

  getTrendingGigs: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiClient(`/gigs/trending?${queryString}`);
  },

  trackGigClick: async (gigId) => apiClient(`/gigs/${gigId}/click`, { method: "POST" }),

  getGigAnalytics: async (gigId) => apiClient(`/gigs/${gigId}/analytics`),

  featureGig: async (gigId) => apiClient(`/gigs/admin/${gigId}/feature`, { method: "POST" }),

  unfeatureGig: async (gigId) => apiClient(`/gigs/admin/${gigId}/unfeature`, { method: "POST" }),

  approveGig: async (gigId) => apiClient(`/gigs/admin/${gigId}/approve`, { method: "POST" }),

  rejectGig: async (gigId, reason) => apiClient(`/gigs/admin/${gigId}/reject`, {
    method: "POST",
    body: JSON.stringify({ reason }),
  }),

  suspendGig: async (gigId, reason) => apiClient(`/gigs/admin/${gigId}/suspend`, {
    method: "POST",
    body: JSON.stringify({ reason }),
  }),

  resumeGig: async (gigId) => apiClient(`/gigs/admin/${gigId}/resume`, { method: "POST" }),
};
...
export const paymentAPI = {
  createPaymentIntent: async (contractId) => {
    return apiClient("/escrow_wallet/payment/deposit", {
      method: "POST",
  ...
  verifyMpesaPayment: async (checkoutRequestId) => {
    return apiClient(`/escrow_wallet/wallet/mpesa/status/${checkoutRequestId}`);
  },
  ...
  releasePayment: async (contractId) => {
    return apiClient("/escrow_wallet/escrow/release", {
      method: "POST",
  ...
  requestRefund: async (contractId, reason) => {
    return apiClient("/escrow_wallet/escrow/refund", {
      method: "POST",
  ...
  getEscrowBalance: async () => {
    return apiClient("/escrow_wallet/wallet");
  },
};
...
export const walletAPI = {
  ...
  getTransactions: async (params = {}) => {
    const queryString = new URLSearchParams(params).toString();
    return apiClient(`/escrow_wallet/wallet/transactions?${queryString}`);
  },

  updateWithdrawalMethod: async (withdrawalMethod, withdrawalDetails) => {
    return apiClient("/escrow_wallet/wallet/withdrawal-method", {
      method: "PATCH",
  ...
  requestWithdrawal: async (amount, phoneNumber) => {
    return apiClient("/escrow_wallet/withdrawal", {
      method: "POST",
  ...
  depositMpesa: async (amount, phoneNumber) => {
    return apiClient("/escrow_wallet/wallet/mpesa/deposit", {
      method: "POST",
  ...
  getPublicFees: async () => {
    return apiClient("/escrow_wallet/fees/public");
  },

  calculateFees: async (amount, context = {}) => {
    return apiClient("/escrow_wallet/fees/calculate", {
      method: "POST",
      body: JSON.stringify({ amount, ...context }),
    });
  },

  getVaultCompliance: async () => apiClient("/escrow_wallet/vault/compliance"),

  initiateStkPush: async ({ amount, phoneNumber, projectId, serviceId, contractId, orderId, idempotencyKey }) => {
    return apiClient("/escrow_wallet/wallet/mpesa/stk-push", {
      method: "POST",
  ...
  paySubscription: async (amount, planId, useWalletBalance = false) => {
    return apiClient("/escrow_wallet/subscriptions/pay", {
      method: "POST",
...
export const messageAPI = {
  getConversations: async () => {
    return apiClient("/chat_messaging/conversations");
  },

  getMessages: async (conversationId) => {
    return apiClient(`/chat_messaging/conversations/${conversationId}/messages`);
  },

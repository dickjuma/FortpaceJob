      ...
      >
        <div className="h-20 flex items-center justify-between px-6 border-b border-zinc-200 dark:border-zinc-800">
          <div className={`flex items-center gap-2 overflow-hidden ${!sidebarOpen && 'justify-center'}`}>
            <div className="w-8 h-8 rounded-lg bg-[#4C1D95] flex items-center justify-center shrink-0 shadow-lg shadow-#4C1D95]/20">
            ...
            return (
              <a 
                ...
                className={`flex items-center gap-3 px-3 py-3 rounded-xl transition-all group relative ${
                  isActive 
                    ? 'bg-[#4C1D95]/5 dark:bg-[#4C1D95]/10 text-[#4C1D95] dark:text-[#4C1D95] font-semibold' 
                    : 'text-zinc-500 hover:bg-surface dark:hover:bg-zinc-800/50 hover:text-zinc-900 dark:hover:text-white font-medium'
                }`}
                title={!sidebarOpen ? item.name : ""}
              >
                <item.icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-[#4C1D95] dark:text-[#4C1D95]' : 'text-zinc-400 group-hover:text-zinc-600 dark:group-hover:text-zinc-300'}`} />
                ...
                {sidebarOpen && item.name === 'Messages' && (
                  <span className="ml-auto w-5 h-5 flex items-center justify-center bg-[#4C1D95] text-white text-[10px] font-bold rounded-full">3</span>
                )}
              </a>
            );
          })}
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="h-20 bg-white/80 dark:bg-surface-dark/80 backdrop-blur-md border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between px-4 sm:px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="hidden sm:flex items-center relative max-w-md w-full">
              ...
              <input 
                ...
                className="w-full pl-10 pr-4 py-2 bg-zinc-100 dark:bg-surface-dark border border-transparent focus:border-[#4C1D95]/20 focus:bg-white dark:focus:bg-zinc-800 rounded-xl outline-none text-sm transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            {userRole === 'client' && (
              <button className="hidden sm:flex items-center gap-2 px-4 py-2 bg-[#4C1D95] hover:bg-[#22C55E] text-white text-sm font-semibold rounded-lg transition-colors shadow-lg shadow-#4C1D95]/20">
                Post Job
              </button>
            )}
            ...
            <div className="relative">
              <button onClick={() => setNotificationsOpen(!notificationsOpen)} className="relative p-2 text-zinc-400 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors">
                ...
                <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-[#4C1D95] rounded-full border-2 border-white dark:border-zinc-950 animate-pulse" />
              </button>
            </div>
            ...
            >
               <div className="h-20 flex items-center justify-between px-6 border-b border-zinc-200 dark:border-zinc-800">
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 rounded-lg bg-[#4C1D95] flex items-center justify-center"><Zap className="w-5 h-5 text-white" /></div>

      ...
      <div className="container mx-auto px-4 md:px-8 py-12">
        <div className="flex flex-col lg:flex-row gap-8 max-w-6xl mx-auto">
          ...
          <div className="w-full lg:w-80 flex-shrink-0 space-y-6">
            ...
            <div className="bg-[#4C1D95]/5 rounded-2xl p-6 border border-[#4C1D95]/20">
              <h4 className="font-bold text-[#4C1D95] mb-2">Current Status</h4>
              <div className="flex items-center gap-2 text-[#4C1D95] font-medium mb-4">
                <div className="w-2.5 h-2.5 bg-success rounded-full animate-pulse"></div>
                Taking new clients
              </div>
              <p className="text-sm text-[#4C1D95]">Sarah has capacity for ~20 hrs/week starting next Monday.</p>
            </div>
              ...
              {[...Array(35)].map((_, i) => {
                ...
                return (
                  <div key={i} className={`h-24 p-2 flex flex-col ${isCurrentMonth ? 'bg-white hover:bg-surface' : 'bg-surface text-zinc-300'} ${isVacation ? 'bg-amber-50/50' : ''} transition-colors cursor-pointer relative`}>
                    <span className={`font-medium w-7 h-7 flex items-center justify-center rounded-full mx-auto ${isToday ? 'bg-[#4C1D95] text-white' : ''}`}>
                      {isCurrentMonth ? day : ''}
                    </span>

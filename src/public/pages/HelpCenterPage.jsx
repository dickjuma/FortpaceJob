...
const CATEGORIES = [
  { title: 'Getting Started', icon: Book, color: 'text-brand-500', bg: 'bg-brand-50 dark:bg-brand-500/10' },
  ...
  { title: 'Account Settings', icon: User, color: 'text-brand-500', bg: 'bg-brand-50 dark:bg-brand-500/10' },
  { title: 'Finding Work', icon: Briefcase, color: 'text-brand-500', bg: 'bg-brand-50 dark:bg-brand-500/10' },
...
export default function HelpCenterPage() {
  ...
  return (
    <div className="min-h-screen bg-surface dark:bg-surface-dark font-sans pb-24">
      ...
      {/* Hero Section */}
      <div className="bg-brand-900 pt-24 pb-32 px-4 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-1/2 h-full bg-gradient-to-l from-brand-600/30 to-transparent"></div>
        ...
        <div className="max-w-4xl mx-auto relative z-10 text-center">
          ...
          <div className="relative max-w-2xl mx-auto">
            <Search className="absolute left-6 top-1/2 -tranzinc-y-1/2 w-6 h-6 text-brand-300" />
            <input 
              type="text" 
              placeholder="Search for articles, tutorials, or topics..." 
              className="w-full bg-white dark:bg-surface-dark border-none rounded-2xl pl-16 pr-4 py-5 text-lg font-bold text-zinc-900 dark:text-white outline-none shadow-2xl focus:ring-4 focus:ring-brand-500/50 transition-all"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
            />
            <button className="absolute right-3 top-1/2 -tranzinc-y-1/2 px-6 py-3 bg-brand-600 hover:bg-brand-700 text-white font-bold rounded-xl transition-colors">
              Search
            </button>
          </div>

          <div className="mt-8 flex flex-wrap justify-center items-center gap-3 text-sm font-medium text-brand-100">
            <span>Popular:</span>
            {['Reset Password', 'Fees', 'Contact Support', 'Withdrawals'].map(term => (
              <span key={term} className="px-3 py-1.5 rounded-full border border-brand-700 hover:bg-brand-800 cursor-pointer transition-colors bg-brand-800/50 backdrop-blur-sm">
                {term}
              </span>
            ))}
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 -mt-16 relative z-20 flex flex-col lg:flex-row gap-8">
              ...
              {ARTICLES.map(article => (
                <button key={article.title} type="button" className="flex items-center justify-between p-3 rounded-xl hover:bg-surface dark:hover:bg-zinc-800 transition-colors group text-left">
                  <div className="flex items-center gap-3">
                    <FileText className="w-4 h-4 text-zinc-400 group-hover:text-brand-500 transition-colors" />
                ...
                <div key={i} className="group cursor-pointer">
                  <div className="relative aspect-video rounded-2xl overflow-hidden mb-3 border border-zinc-200 dark:border-zinc-800 bg-surface-dark">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="w-12 h-12 bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center group-hover:bg-brand-500 transition-colors">
                        <PlayCircle className="w-6 h-6 text-white" />
                      </div>
                    </div>
                  </div>
                  <h3 className="font-bold text-zinc-900 dark:text-white text-sm group-hover:text-brand-600 transition-colors">Getting started with Forte</h3>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="w-full lg:w-80 shrink-0 space-y-6">
          ...
          <div className="bg-gradient-to-br from-brand-600 to-indigo-600 rounded-3xl p-6 shadow-md text-white relative overflow-hidden">
            ...
            <button className="w-full py-3 bg-white text-brand-600 font-bold rounded-xl shadow-sm transition-transform hover:scale-105">
              Ask AI Assistant
            </button>
          </div>
          ...
          <div className="bg-white dark:bg-surface-dark rounded-3xl border border-zinc-200 dark:border-zinc-800 p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <button className="w-full p-4 border border-zinc-200 dark:border-zinc-700 rounded-2xl flex items-center gap-4 hover:border-brand-500 transition-colors group text-left">
                <div className="w-10 h-10 bg-surface dark:bg-zinc-800 rounded-full flex items-center justify-center shrink-0 group-hover:bg-brand-50 dark:group-hover:bg-brand-500/10 transition-colors">
                  <MessageCircle className="w-5 h-5 text-zinc-400 group-hover:text-brand-500" />
                </div>
                <div>
              ...
              <button className="w-full p-4 border border-zinc-200 dark:border-zinc-700 rounded-2xl flex items-center gap-4 hover:border-brand-500 transition-colors group text-left">
                <div className="w-10 h-10 bg-surface dark:bg-zinc-800 rounded-full flex items-center justify-center shrink-0 group-hover:bg-brand-50 dark:group-hover:bg-brand-500/10 transition-colors">
                  <PhoneCall className="w-5 h-5 text-zinc-400 group-hover:text-brand-500" />
                </div>
                <div>
              ...
              <button className="w-full p-4 border border-zinc-200 dark:border-zinc-700 rounded-2xl flex items-center gap-4 hover:border-brand-500 transition-colors group text-left">
                <div className="w-10 h-10 bg-surface dark:bg-zinc-800 rounded-full flex items-center justify-center shrink-0 group-hover:bg-brand-50 dark:group-hover:bg-brand-500/10 transition-colors">
                  <HelpCircle className="w-5 h-5 text-zinc-400 group-hover:text-brand-500" />
                </div>
                <div>

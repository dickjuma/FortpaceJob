    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans">
      ...
      {loading ? (
        <div className="py-20 text-center">
          <Loader2 className="w-10 h-10 mx-auto animate-spin text-[#14a800]" />
        </div>
      ) : stories.length === 0 ? (
        <div className="py-20 text-center bg-white border border-dashed border-zinc-300 rounded-2xl">
          ...
          <Link to="/find-talent" className="inline-block mt-4 text-[#14a800] font-bold hover:underline">
            Explore talent
          </Link>
        </div>
      ) : (
        <>
          {featured ? (
            <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden mb-12 shadow-sm">
              <div className="p-8 md:p-12">
                <span className="text-xs font-bold text-[#14a800] uppercase tracking-wider">{featured.category}</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {stories.slice(1).map((story) => (
              <article key={story.id} className="bg-white border border-zinc-200 rounded-2xl p-6 hover:shadow-md transition-shadow">
                ...
                {story.stat ? <div className="text-[#14a800] font-black text-lg mt-1">{story.stat}</div> : null}
          ...
          <div className="text-center mt-12">
            <Link to="/register" className="inline-flex items-center gap-2 px-8 py-4 bg-[#14a800] text-white font-bold rounded-xl hover:bg-[#118a00]">

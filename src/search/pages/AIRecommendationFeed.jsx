    ...
    <div className="max-w-5xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="bg-gradient-to-r from-[#14a800] to-[#118a00] rounded-3xl p-8 mb-8 text-white relative overflow-hidden shadow-xl">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between">
          ...
          <Link
            to="/find-talent"
            className="flex-shrink-0 px-6 py-3 bg-white text-[#14a800] font-bold rounded-xl shadow-lg hover:bg-zinc-100 transition-colors flex items-center"
          >
        ...
        <div className="space-y-6">
          ...
          {matches.map((match, idx) => (
            <motion.div
              ...
              key={match.id}
              className="bg-white dark:bg-gray-900 rounded-2xl border-2 border-[#14a800]/20 shadow-sm overflow-hidden flex flex-col md:flex-row relative"
            >
              <div className="md:w-48 bg-[#14a800]/5 p-6 flex flex-col items-center justify-center border-r border-[#14a800]/20">
                <span className="text-2xl font-bold text-gray-900 dark:text-white">{match.matchScore}%</span>
                <span className="text-sm font-bold text-[#14a800] uppercase tracking-wide">Match</span>
              </div>
              <div className="p-6 flex-1">
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">{match.name}</h3>
                <p className="text-[#14a800] font-medium">{match.role}</p>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{match.reason}</p>
                <div className="mt-4 flex gap-3">
                  <Link
                    to={`/freelancer/${match.id}`}
                    className="inline-flex items-center gap-2 px-4 py-2 bg-[#14a800] text-white text-sm font-bold rounded-lg"
                  >

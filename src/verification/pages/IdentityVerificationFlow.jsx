  ...
  return (
    <div className="max-w-3xl mx-auto py-12 px-4 sm:px-6 font-sans">
      <div className="text-center mb-10">
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-[#4C1D95]/10 dark:bg-[#4C1D95]/30 text-[#4C1D95] dark:text-[#4C1D95] mb-4">
      ...
      <div className="flex justify-between items-center mb-12 relative">
        ...
        {[1, 2, 3, 4].map((s) => (
          <div key={s} className={`flex items-center justify-center w-10 h-10 rounded-full font-bold ${step >= s ? 'bg-[#4C1D95] text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-500'}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {['Passport', 'Driver License', 'National ID'].map((type) => (
                <button
                  ...
                  className={`p-4 border rounded-xl text-left transition-colors ${documentType === type ? 'border-[#4C1D95]/20 bg-[#4C1D95]/5 dark:bg-[#4C1D95]/20' : 'border-gray-200 dark:border-gray-800 hover:border-[#4C1D95]/20'}`}
                >
        ...
        <div className="mt-8 flex justify-end">
          {step < 4 && (
            <button
              ...
              className="px-6 py-3 bg-[#4C1D95] hover:bg-[#22C55E] text-white rounded-xl font-medium transition-colors flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
            >
